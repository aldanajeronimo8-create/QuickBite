...
The Supabase `quickbite-health` Edge Function records database availability and latency in `system_health_checks`, returning HTTP 503 when the check cannot complete. Automated health monitoring and PostgreSQL backups are configured in `.github/workflows/health-check.yml` and `.github/workflows/backup.yml`. See [BACKUP_RECOVERY_RUNBOOK.md](BACKUP_RECOVERY_RUNBOOK.md) for the required encrypted GitHub secrets, checksum verification and the manual recovery procedure.
