...
/* Table schema:
CREATE TABLE kv_store_94291f1a (
  key TEXT NOT NULL PRIMARY KEY,
  value JSONB NOT NULL
);
*/

// View the backing table in your own Supabase project dashboard.
...
export const del = async (key: string): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_94291f1a").delete().eq("key", key);
  if (error) {
    throw new Error(error.message);
  }
};
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_94291f1a").delete().in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
};
