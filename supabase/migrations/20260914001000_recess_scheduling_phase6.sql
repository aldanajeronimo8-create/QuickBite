...
begin
 ...
 select jsonb_build_object('id',q.id,'name',q.name,'start_time',q.start_time,'end_time',q.end_time,'minutes_remaining',greatest(0,(extract(hour from q.end_time)::int*60+extract(minute from q.end_time)::int)-v_minutes),'scope_level',q.scope_level) into v_active
 ...
 select jsonb_build_object('id',q.id,'name',q.name,'start_time',q.start_time,'end_time',q.end_time,'starts_in_minutes',q.starts_in_minutes,'scope_level',q.scope_level) into v_next
...
revoke execute on function public.get_student_recess_status() from anon;
