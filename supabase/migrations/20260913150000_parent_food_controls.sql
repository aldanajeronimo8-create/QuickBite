...
create index if not exists parent_food_blocks_student_idx on public.parent_food_blocks(student_user_id);
create index if not exists parent_food_blocks_parent_student_idx on public.parent_food_blocks(parent_user_id, student_user_id);
...
create policy parent_food_blocks_no_direct_access on public.parent_food_blocks for all using (false) with check (false);
...
begin
  ...
  left join public.parent_food_blocks b
   ...
   and (v_is_admin or b.parent_user_id = auth.uid())
...
begin
  ...
  if p_blocked then
    ...
    on conflict (parent_user_id, student_user_id, product_id)
...
begin
  ...
  if v_user_id is not null and exists (
    ...
    where b.student_user_id = v_user_id and b.product_id = new.product_id
  ) then
...
create policy products_public_read_available on public.products for select
using (
  is_admin()
  or (
    available = true
