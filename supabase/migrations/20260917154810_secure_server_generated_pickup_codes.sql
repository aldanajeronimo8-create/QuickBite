...
CREATE UNIQUE INDEX IF NOT EXISTS orders_pickup_code_unique_idx
  ON public.orders (upper(trim(pickup_code)))
...
begin
  ...
  begin
    ...
    values(v_order_id,p_user_id,0,v_initial_status,p_payment_method,v_initial_payment_status,v_order_number,v_pickup_code,p_estimated_minutes,p_payment_reference,nullif(trim(p_notes),''),nullif(trim(p_notes),''),p_request_id,case when v_windows_enabled then v_slot.id else null end);
