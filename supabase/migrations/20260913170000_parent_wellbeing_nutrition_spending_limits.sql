create table if not exists public.product_nutrition (
  ...
  calories numeric(7,1), protein_g numeric(7,1), carbohydrates_g numeric(7,1), fat_g numeric(7,1), fiber_g numeric(7,1), ingredients text, allergens text,
  vegetarian boolean not null default false, healthy_choice boolean not null default false, updated_at timestamptz not null default now()
);
create table if not exists public.student_spending_limits (
  ...
  daily_limit numeric(12,2), weekly_limit numeric(12,2), monthly_limit numeric(12,2), updated_at timestamptz not null default now()
);
...
create policy product_nutrition_public_read on public.product_nutrition for select using (true);
drop policy if exists product_nutrition_admin_write on public.product_nutrition;
create policy product_nutrition_admin_write on public.product_nutrition for all using (public.is_admin()) with check (public.is_admin());
drop policy if exists spending_limits_parent_read on public.student_spending_limits;
create policy spending_limits_parent_read on public.student_spending_limits for select using (student_user_id = public.effective_student_user_id() or exists (select 1 from public.parent_student_links l where l.parent_user_id = auth.uid() and l.student_user_id = student_spending_limits.student_user_id and l.active = true));
create or replace function public.get_parent_spending_limit(p_student_user_id uuid)
returns public.student_spending_limits language plpgsql stable security definer set search_path=public as $$
declare result public.student_spending_limits; begin
  if not exists (select 1 from public.parent_student_links where parent_user_id=auth.uid() and student_user_id=p_student_user_id and active=true) then raise exception 'student_not_linked'; end if;
  select * into result from public.student_spending_limits where student_user_id=p_student_user_id; return result;
end; $$;
create or replace function public.set_parent_spending_limits(p_student_user_id uuid,p_daily_limit numeric,p_weekly_limit numeric,p_monthly_limit numeric)
returns public.student_spending_limits language plpgsql security definer set search_path=public as $$
declare result public.student_spending_limits; begin
  if not exists (select 1 from public.parent_student_links where parent_user_id=auth.uid() and student_user_id=p_student_user_id and active=true) then raise exception 'student_not_linked'; end if;
  if (p_daily_limit is not null and p_daily_limit < 0) or (p_weekly_limit is not null and p_weekly_limit < 0) or (p_monthly_limit is not null and p_monthly_limit < 0) then raise exception 'invalid_spending_limit'; end if;
  insert into public.student_spending_limits(student_user_id,daily_limit,weekly_limit,monthly_limit) values(p_student_user_id,p_daily_limit,p_weekly_limit,p_monthly_limit)
  on conflict(student_user_id) do update set daily_limit=excluded.daily_limit,weekly_limit=excluded.weekly_limit,monthly_limit=excluded.monthly_limit,updated_at=now() returning * into result; return result;
end; $$;
...
declare lim public.student_spending_limits; today_total numeric; week_total numeric; month_total numeric; begin
  select * into lim from public.student_spending_limits where student_user_id=new.user_id;
  if lim.student_user_id is null or new.status in ('cancelled','rejected') then return new; end if;
...
create trigger trg_enforce_student_spending_limits before insert on public.orders for each row execute function public.enforce_student_spending_limits();
