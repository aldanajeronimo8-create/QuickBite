...
create policy spending_limits_parent_read on public.student_spending_limits for select using (
  student_user_id = public.effective_student_user_id()
  or exists (
    select 1 from public.parent_student_links l
    where l.parent_user_id = auth.uid() and l.student_user_id = student_spending_limits.student_user_id and l.active = true
  )
);
...
as $$
declare result public.student_spending_limits;
begin
  if not exists (select 1 from public.parent_student_links where parent_user_id=auth.uid() and student_user_id=p_student_user_id and active=true) then
    raise exception 'student_not_linked';
  end if;
  select * into result from public.student_spending_limits where student_user_id=p_student_user_id;
  return result;
end;
$$;
