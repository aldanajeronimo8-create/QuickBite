...
DO $$
BEGIN
  ALTER TABLE public.loyalty_rewards DROP CONSTRAINT IF EXISTS loyalty_rewards_points_required_check;
  ALTER TABLE public.loyalty_rewards
...
AS $$
BEGIN
  IF NEW.points_cost IS NULL AND NEW.points_required IS NULL THEN RAISE EXCEPTION 'points_required'; END IF;
  ...
  ELSE
    NEW.points_required := NEW.points_cost;
  END IF;
  IF NEW.points_cost <= 0 OR NEW.points_required <= 0 THEN RAISE EXCEPTION 'points_required'; END IF;
  RETURN NEW;
END;
$$;
...
BEFORE INSERT OR UPDATE OF points_cost, points_required ON public.loyalty_rewards
FOR EACH ROW EXECUTE FUNCTION public.sync_loyalty_reward_points();
...
BEGIN
  ...
  IF v_reward_cost IS NULL OR v_reward_cost <= 0 THEN RAISE EXCEPTION 'invalid_reward_points'; END IF;
  IF v_reward.stock IS NOT NULL AND v_reward.stock <= 0 THEN RAISE EXCEPTION 'reward_out_of_stock'; END IF;

  SELECT GREATEST(
    0,
    COALESCE(SUM(FLOOR(o.total / v_currency_amount) * v_points_per_amount) FILTER (WHERE o.payment_status = 'confirmed'), 0)::INTEGER
    - COALESCE((SELECT SUM(points_spent) FROM public.loyalty_redemptions WHERE user_id = v_user_id AND status <> 'cancelled'), 0)::INTEGER
  ) INTO v_points
  FROM public.orders o
  WHERE o.user_id = v_user_id;

  IF v_points < v_reward_cost THEN RAISE EXCEPTION 'insufficient_loyalty_points'; END IF;
...
-- Only this RPC can change the comment for a student's own order.
DROP POLICY IF EXISTS orders_update_own_comment ON public.orders;
CREATE OR REPLACE FUNCTION public.update_own_order_comment(p_order_number TEXT, p_comment TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.update_own_order_comment(TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_own_order_comment(TEXT, TEXT) TO authenticated;

CREATE INDEX IF NOT EXISTS idx_orders_user_created_at_comment ON public.orders(user_id, created_at DESC);
