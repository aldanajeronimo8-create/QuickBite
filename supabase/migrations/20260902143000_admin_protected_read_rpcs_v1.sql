...
begin
  ...
  return query
    select s.id, s.enabled, s.points_per_currency_unit, s.updated_at
...
begin
  if auth.uid() is null or not public.is_admin() then raise exception 'unauthorized'; end if;
  return query
    select c.user_id, c.student_name, c.guardian_name, c.guardian_relationship,
           c.guardian_email, c.student_acknowledged, c.guardian_authorized,
           c.privacy_policy_version, c.consent_at
    from public.student_data_consents c
...
create or replace function public.admin_list_loyalty_redemptions(p_limit integer default 50)
returns jsonb
language plpgsql
security definer
set search_path = 'public'
...
begin
  ...
  select coalesce(jsonb_agg(row_to_json(x) order by x.created_at desc), '[]'::jsonb)
    into v_result
  from (
    select r.id, r.user_id, r.reward_id, r.product_id, r.points_spent,
           r.redemption_code, r.status, r.created_at, r.admin_hidden, r.fulfilled_at,
           jsonb_build_object('id', p.id, 'full_name', p.full_name, 'email', p.email) as "user",
           jsonb_build_object('id', rw.id, 'title', rw.title) as reward
    from public.loyalty_redemptions r
    left join public.profiles p on p.id = r.user_id
    left join public.loyalty_rewards rw on rw.id = r.reward_id
...
$$;
revoke execute on function public.admin_list_loyalty_redemptions(integer) from public, anon;
grant execute on function public.admin_list_loyalty_redemptions(integer) to authenticated;
...
begin
  ...
  from (
    select o.id, o.user_id, o.total, o.status, o.payment_method, o.payment_status,
           o.order_number, o.created_at, o.admin_hidden, o.pickup_code,
           o.estimated_minutes, o.payment_reference, o.notes, o.student_comment,
           case when p.id is null then null else jsonb_build_object(
             'id', p.id, 'email', p.email, 'full_name', p.full_name, 'role', p.role, 'ti', p.ti, 'created_at', p.created_at
           ) end as "user",
           coalesce((
             select jsonb_agg(jsonb_build_object(
               ...
               'product', case when pr.id is null then null else jsonb_build_object(
                 ...
                 'image_url', pr.image_url, 'category_id', pr.category_id, 'stock', pr.stock,
                 'available', pr.available, 'created_at', pr.created_at,
                 'category', case when c.id is null then null else jsonb_build_object('id', c.id, 'name', c.name, 'description', c.description, 'created_at', c.created_at) end
               ) end
             ) order by oi.id)
