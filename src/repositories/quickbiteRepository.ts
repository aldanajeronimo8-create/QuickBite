...
function orderStatusRpcError(error: unknown) {
  ...
  if (/order_not_found/i.test(message)) {
    return new Error('El pedido ya no existe o no esta disponible.');
  }
...
function loyaltyRpcError(error: unknown) {
  ...
  if (/insufficient_points/i.test(message)) return new Error('No tienes puntos suficientes para este canje.');
  return new Error(message);
}
...
export async function markUserNotificationsRead(notificationIds?: string[]) {
  const { error } = await requireSupabaseClient().rpc('mark_notifications_read', {
...
export async function getLoyaltySettings() {
  const { data, error } = await requireSupabaseClient()
    ...
    .select('id,enabled,points_per_currency_unit,updated_at')
    .eq('id', true)
    .single();
  if (error) throw error;
...
export async function listUserLoyaltyRedemptions(userId: string) {
  const { data, error } = await requireSupabaseClient()
    ...
    .select('*, reward:loyalty_rewards(id,title,product:products(name))')
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
    .limit(20);
  if (error) throw error;
...
export async function listAdminLoyaltyRedemptions(limit = 50) {
  const { data, error } = await requireSupabaseClient()
    .from('loyalty_redemptions')
    .select('*, user:profiles(id,full_name,email), reward:loyalty_rewards(id,title)')
    .order('created_at', { ascending: false })
    .limit(Math.min(Math.max(limit, 1), 100));
  if (error) throw error;
...
export async function fulfillLoyaltyRedemption(redemptionId: string, redemptionCode: string) {
  const { error } = await requireSupabaseClient().rpc('admin_fulfill_loyalty_redemption', {
...
export async function writeAudit(entry: {
  ...
  const { error } = await requireSupabaseClient().from('audit_logs').insert({
    action: entry.action,
