...
export function StudentMenuPage() {
  ...
  return (
    <div className="min-h-screen bg-[#fff7e8] pb-24 text-slate-900">
        ...
        <div className="mx-auto max-w-6xl px-5 pb-5 pt-5 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="grid h-11 w-11 place-items-center rounded-2xl bg-orange-500 shadow-lg shadow-orange-500/30"><Coffee className="h-6 w-6" /></div>
              <div><p className="text-lg font-black leading-none">QuickBite</p><p className="text-xs text-orange-100">{student.name}{student.grade ? ` - ${student.grade}` : ''}</p></div>
            </div>
          ...
          <div className="mt-5 rounded-[2rem] bg-gradient-to-r from-orange-500 to-amber-400 p-4 text-white">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-100">Recreo inteligente</p><h1 className="mt-1 text-2xl font-black">Pide ahora, recoge sin fila</h1></div>{rewardsEnabled && <Badge className="bg-white text-orange-700">{loyalty.availablePoints} pts</Badge>}</div>
            <div className="mt-4 flex gap-2 overflow-x-auto"><span className="rounded-full bg-white/20 px-3 py-1 text-xs">Inventario en vivo</span><span className="rounded-full bg-white/20 px-3 py-1 text-xs">Tiempo promedio: 12 min</span></div>
          </div>
        </div>
      </header>

      <nav className={`sticky top-[190px] z-10 mx-5 mt-5 grid ${rewardsEnabled ? 'grid-cols-3' : 'grid-cols-2'} rounded-3xl bg-white p-1.5 shadow-lg ring-1 ring-orange-100`}>
        {navigationTabs.map(([id, Icon, label]) => (
          <button key={id} onClick={() => setTab(id)} className={`flex items-center justify-center gap-1 rounded-xl py-2 text-sm font-bold transition ${activeTab === id ? 'bg-[#14213d] text-white' : 'text-slate-600'}`}><Icon className="h-4 w-4" />{label}</button>
        ))}
      </nav>

      {activeTab === 'menu' && <main className="mx-auto max-w-6xl px-5 pt-6 lg:px-8">
        <div className="relative mb-3"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar empanadas, jugos, almuerzos..." className="w-full rounded-2xl border border-orange-100 bg-white py-3 pl-11 pr-4 outline-none focus:ring-2 focus:ring-orange-300" /></div>
        <div className="mb-4 flex gap-2 overflow-x-auto pb-1"><button onClick={() => setSelectedCat(null)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${!selectedCat ? 'bg-orange-500 text-white' : 'bg-white text-slate-600'}`}>Todo</button>{categories.map((cat) => <button key={cat.id} onClick={() => setSelectedCat(cat.id)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${selectedCat === cat.id ? 'bg-orange-500 text-white' : 'bg-white text-slate-600'}`}>{cat.name}</button>)}</div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{availableProducts.map((product) => { const qty = cartQty(product.id); return <article key={product.id} className="overflow-hidden rounded-3xl border border-orange-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl"><div className="relative"><img src={product.image_url} alt={product.name} className="h-44 w-full object-cover" /><span className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-1 text-[11px] font-bold text-slate-700">Stock {product.stock}</span></div><div className="p-4"><p className="min-h-10 text-sm font-black leading-tight">{product.name}</p><p className="line-clamp-1 text-xs text-slate-600">{product.description}</p><p className="mt-2 text-lg font-black text-orange-500">${fmt(product.price)}</p>{qty === 0 ? <button onClick={() => addToCart(product)} className="mt-2 flex w-full items-center justify-center gap-1 rounded-2xl bg-[#14213d] py-2 text-sm font-bold text-white"><Plus className="h-4 w-4" />Agregar</button> : <div className="mt-2 flex items-center justify-between rounded-2xl bg-orange-50 p-1"><button onClick={() => removeFromCart(product.id)} className="grid h-8 w-8 place-items-center rounded-full bg-white text-orange-600"><Minus className="h-4 w-4" /></button><span className="font-black">{qty}</span><button onClick={() => addToCart(product)} className="grid h-8 w-8 place-items-center rounded-full bg-orange-500 text-white"><Plus className="h-4 w-4" /></button></div>}</div></article>; })}</div>
      </main>}
      ...
      {cartCount > 0 && !showCart && <div className="fixed bottom-4 left-4 right-4 z-30"><button onClick={() => { setShowCart(true); setPayStep('cart'); }} className="flex w-full items-center justify-between rounded-3xl bg-orange-500 px-5 py-4 font-black text-white shadow-2xl shadow-orange-500/30"><span>{cartCount} items</span><span>Ver pedido</span><span>${fmt(cartGrandTotal)}</span></button></div>}
...
function CartSheet({
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col justify-end bg-slate-950/50" onClick={onClose}>
      <section className="max-h-[88vh] overflow-y-auto rounded-t-[2rem] bg-white" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-5 py-4">
          <h2 className="text-xl font-black">{payStep === 'receipt' ? 'Recibo digital' : payStep === 'payment' ? 'Confirmar pago' : 'Tu pedido'}</h2>
          <button onClick={onClose} className="text-sm font-bold text-slate-400">Cerrar</button>
        </div>
        <div className="space-y-4 p-5">
          ...
          {payStep === 'payment' && <>
            <div className="rounded-3xl bg-slate-50 p-4">
              <div className="mb-3 flex items-center gap-2"><CreditCard className="h-5 w-5 text-orange-500" /><p className="font-black">Pago - {paymentOptions.find((p) => p.value === paymentMethod)?.label}</p></div>
              <p className="text-sm text-slate-600">Referencia: <b>{reference}</b></p>
              <p className="text-sm text-slate-600">Total a registrar: <b>${fmt(total)}</b></p>
              <p className="mt-4 rounded-2xl bg-emerald-50 p-3 text-sm text-emerald-700">{paymentMethod === 'cash' ? 'El pedido quedara pendiente hasta que el admin confirme el efectivo.' : 'El pedido quedara confirmado y sincronizado en Supabase.'}</p>
            </div>
            <div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => onSetPayStep('cart')} className="rounded-2xl py-6">Volver</Button><Button disabled={placing} onClick={onPay} className="rounded-2xl bg-orange-500 py-6 text-white">{placing ? 'Procesando...' : 'Pagar'}</Button></div>
          </>}
          {payStep === 'receipt' && lastReceipt && <div className="rounded-3xl border border-dashed border-orange-300 bg-orange-50 p-5 text-center"><CheckCircle2 className="mx-auto mb-3 h-12 w-12 text-emerald-500" /><p className="text-sm font-bold text-slate-600">Pedido creado</p><p className="text-3xl font-black text-[#14213d]">{lastReceipt.orderNumber}</p><p className="mt-3 text-sm">Codigo de recogida</p><p className="text-4xl font-black tracking-[0.2em] text-orange-500">{lastReceipt.pickup}</p><p className="mt-3 text-xs text-slate-600">Ref. pago: {lastReceipt.reference}</p><Button onClick={() => { onClose(); onSetTab('orders'); onSetPayStep('cart'); }} className="mt-5 w-full rounded-2xl bg-[#14213d] py-6 text-white">Ver historial</Button></div>}
        </div>
      </section>
    </div>
  );
...
function Summary({ subtotal, fee, total }: { subtotal: number; fee: number; total: number }) {
  return <div className="space-y-2 rounded-3xl bg-slate-50 p-4 text-sm"><div className="flex justify-between"><span>Subtotal</span><b>${fmt(subtotal)}</b></div><div className="flex justify-between"><span>Cargo de pago</span><b>{fee ? `$${fmt(fee)}` : 'Gratis'}</b></div><div className="flex justify-between border-t pt-2 text-lg font-black"><span>Total</span><span className="text-orange-500">${fmt(total)}</span></div></div>;
}

function Empty({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }) {
  return <div className="rounded-3xl bg-white p-8 text-center shadow-sm"><Icon className="mx-auto mb-3 h-12 w-12 text-orange-300" /><h3 className="font-black">{title}</h3><p className="mt-1 text-sm text-slate-600">{text}</p></div>;
}
...
function OrderCard({ order }: { order: Order }) {
  ...
  return <article className="rounded-3xl bg-white p-4 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-xs font-bold text-slate-400">#{order.order_number}</p><h3 className="font-black">{label}</h3></div><Badge className={order.payment_status === 'confirmed' ? 'bg-emerald-500 text-white' : order.payment_status === 'rejected' ? 'bg-red-500 text-white' : 'bg-yellow-500 text-white'}>{order.payment_status === 'confirmed' ? 'Pagado' : order.payment_status === 'rejected' ? 'Rechazado' : 'Pendiente'}</Badge></div><div className="mt-3 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-orange-50 text-orange-500"><Icon className="h-5 w-5" /></div><div className="flex-1"><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-orange-500" style={{ width: `${progress}%` }} /></div><p className="mt-1 text-xs text-slate-600">Recogida: <b>{order.pickup_code ?? '----'}</b> - {order.estimated_minutes ?? 12} min</p></div></div><div className="mt-3 flex justify-between border-t pt-3 text-sm"><span className="text-slate-600">Total</span><b className="text-orange-500">${fmt(order.total)}</b></div></article>;
}
