...
function statusData(order: Order): [string, number, LucideIcon] { if (order.status === 'pending') return ['Pendiente', 0, Clock3]; if (order.status === 'preparing') return ['Preparando', 1, Utensils]; if (order.status === 'ready') return ['Listo', 2, PackageCheck]; if (order.status === 'completed') return ['Entregado', 3, CheckCircle2]; return ['Cancelado', 0, ReceiptText]; }
