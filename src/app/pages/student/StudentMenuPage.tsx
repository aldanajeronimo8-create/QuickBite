...
function CartSheet({ cart, cartTotal, total, lastReceipt, payStep, paymentMethod, placing, reference, tip, walletBalance, onAdd, onClose, onPay, onRemove, onSelectPayment, onSetPayStep, onSetTab, onTip }: { cart: CartItem[]; cartTotal: number; total: number; lastReceipt: { orderNumber: string; reference: string; pickup: string; paidWithCredits: boolean } | null; payStep: PayStep; paymentMethod: Order['payment_method']; placing: boolean; reference: string; tip: string; walletBalance: number; onAdd: (product: Product) => void; onClose: () => void; onPay: () => void; onRemove: (id: string) => void; onSelectPayment: (method: Order['payment_method']) => void; onSetPayStep: (step: PayStep) => void; onSetTab: (tab: Tab) => void; onTip: (tip: string) => void; }) {
  ...
  return <div className="fixed inset-0 z-40 flex flex-col justify-end bg-slate-950/50" onClick={onClose}><section className="max-h-[88vh] overflow-y-auto rounded-t-[2rem] bg-white" onClick={(e) => e.stopPropagation()}><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-5 py-4"><h2 className="text-xl font-black">{payStep === 'receipt' ? 'Recibo digital' : payStep === 'payment' ? 'Confirmar pago' : 'Tu pedido'}</h2><button onClick={onClose} className="text-sm font-bold text-slate-400">Cerrar</button></div><div className="space-y-4 p-5">
    {payStep === 'cart' && <><div className="space-y-3">{cart.map((item) => <div key={item.id} className="flex items-center gap-3"><img src={item.image_url} alt={item.name} className="h-14 w-14 rounded-2xl object-cover" /><div className="flex-1"><p className="text-sm font-black">{item.name}</p><p className="text-sm font-bold text-green-800">${fmt(item.price)} x {item.qty}</p></div><button onClick={() => onRemove(item.id)} className="rounded-full bg-green-50 p-2 text-green-700"><Minus className="h-4 w-4" /></button><button onClick={() => onAdd(item)} className="rounded-full bg-green-600 p-2 text-white hover:bg-green-700"><Plus className="h-4 w-4" /></button></div>)}</div><div><p className="mb-2 text-sm font-black">Método de pago</p><div className="grid grid-cols-2 gap-2">{paymentOptions.map((opt) => <button key={opt.value} onClick={() => onSelectPayment(opt.value)} className={`rounded-2xl border p-3 text-left ${paymentMethod === opt.value ? 'border-green-600 bg-green-50' : 'border-slate-200'}`}><span className={`mb-2 block h-2 w-8 rounded-full ${opt.accent}`} /><span className="block text-sm font-black">{opt.label}</span><span className="text-xs text-slate-600">{opt.hint}</span>{opt.value === 'credits' && <span className={`mt-2 block text-xs font-bold ${creditsInsufficient ? 'text-rose-600' : 'text-violet-700'}`}>Disponible: ${fmt(walletBalance)}</span>}</button>)}</div>{creditsSelected && <div className={`mt-2 rounded-2xl p-3 text-sm ${creditsInsufficient ? 'bg-rose-50 text-rose-700' : 'bg-violet-50 text-violet-800'}`}><div className="flex justify-between"><span>Créditos disponibles</span><b>${fmt(walletBalance)}</b></div><div className="mt-1 flex justify-between"><span>Total del pedido</span><b>${fmt(total)}</b></div>{creditsInsufficient ? <p className="mt-2 font-bold">Te faltan ${fmt(total - walletBalance)} para completar esta compra.</p> : <p className="mt-2 font-bold">Al pagar, se descontarán ${fmt(total)} de tu billetera.</p>}</div>}</div><textarea value={tip} onChange={(e) => onTip(e.target.value)} className="w-full rounded-2xl border border-slate-200 p-3 text-sm outline-none focus:ring-2 focus:ring-green-200" rows={2} placeholder="Notas para cafeteria" /><Summary subtotal={cartTotal} total={total} /><Button disabled={creditsInsufficient} onClick={() => onSetPayStep('payment')} className="w-full rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700 disabled:bg-slate-300">Continuar al pago <ChevronRight className="ml-1 h-5 w-5" /></Button></>}
    {payStep === 'payment' && <><div className="rounded-3xl bg-slate-50 p-4"><div className="mb-3 flex items-center gap-2"><CreditCard className="h-5 w-5 text-green-600" /><p className="font-black">Pago - {paymentOptions.find((p) => p.value === paymentMethod)?.label}</p></div><p className="text-sm text-slate-600">Referencia: <b>{reference}</b></p><p className="text-sm text-slate-600">Total a registrar: <b>${fmt(total)}</b></p>{creditsSelected ? <><p className="text-sm text-slate-600">Créditos disponibles: <b>${fmt(walletBalance)}</b></p><p className="mt-4 rounded-2xl bg-violet-50 p-3 text-sm text-violet-800">Al confirmar, QuickBite descontará el total de tu saldo de créditos de forma segura y atómica. El pedido quedará pagado inmediatamente.</p></> : <p className="mt-4 rounded-2xl bg-amber-50 p-3 text-sm text-amber-700">El pago quedará pendiente hasta que un administrador lo revise y apruebe. El QR de recogida se habilitará únicamente después de la aprobación.</p>}</div><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => onSetPayStep('cart')} className="rounded-2xl py-6">Volver</Button><Button disabled={placing || creditsInsufficient} onClick={onPay} className="rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700 disabled:bg-slate-300">{placing ? 'Procesando...' : creditsSelected ? 'Pagar con créditos' : 'Enviar para aprobación'}</Button></div></>}
    {payStep === 'receipt' && lastReceipt && <div className={`rounded-3xl border border-dashed p-5 text-center ${lastReceipt.paidWithCredits ? 'border-violet-300 bg-violet-50' : 'border-amber-300 bg-amber-50'}`}><Clock3 className={`mx-auto mb-3 h-12 w-12 ${lastReceipt.paidWithCredits ? 'text-violet-600' : 'text-amber-600'}`} /><p className="text-sm font-bold text-slate-600">Pedido enviado</p><p className="text-3xl font-black text-slate-900">{lastReceipt.orderNumber}</p><p className={`mt-3 text-sm font-bold ${lastReceipt.paidWithCredits ? 'text-violet-700' : 'text-amber-700'}`}>{lastReceipt.paidWithCredits ? 'Pago con créditos confirmado' : 'Pago pendiente de aprobación'}</p><p className="mt-2 text-sm text-slate-600">{lastReceipt.paidWithCredits ? `Se descontaron los créditos del pedido. Tu QR de recogida ya está disponible en Historial.` : 'El código QR y el código de recogida estarán disponibles cuando Admin apruebe el pago.'}</p><p className="mt-3 text-xs text-slate-600">Ref. pago: {lastReceipt.reference}</p><Button onClick={() => { onClose(); onSetTab('orders'); onSetPayStep('cart'); }} className="mt-5 w-full rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700">Ver historial</Button></div>}
  </div></section></div>;
}
