...
const PAGE = 'min-h-screen bg-[#0D111D] pb-24 text-white';
const CONTAINER = 'mx-auto w-full max-w-5xl px-4 sm:px-6';
const SURFACE = 'bg-[#131B2E] border border-slate-800';
const INNER = 'bg-[#0D111D] border border-slate-800';
      ...
      <div className={`${CONTAINER} pt-4`}>
        <div className="grid gap-3 sm:grid-cols-2">
          <section className={`${SURFACE} rounded-2xl p-4`}>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-400">Recreo inteligente</p>
            <div className="mt-1 flex items-center justify-between gap-3">
              <h1 className="text-xl font-black text-white">Pide ahora, recoge sin fila</h1>
              <span className="hidden rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-400 sm:inline-flex">Inventario en vivo</span>
            </div>
            <p className="mt-2 text-sm text-slate-400">Consulta disponibilidad y prepara tu pedido antes del recreo.</p>
          </section>
          <section className={`${SURFACE} rounded-2xl p-4`}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Créditos disponibles</p>
                <p className="mt-1 text-2xl font-black text-white">${fmt(walletBalance)}</p>
              </div>
              <span className="rounded-full border border-slate-700 bg-[#0D111D] px-3 py-1 text-xs font-bold text-slate-300">Billetera</span>
            </div>
          </section>
        </div>
      </div>
        ...
        <main className={`${CONTAINER} pt-4`}>
          <section className={`${SURFACE} rounded-2xl p-3 sm:p-4`}>
            <div className="relative">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar empanadas, jugos, almuerzos..." className="w-full rounded-xl border border-slate-800 bg-[#0D111D] py-3 pl-11 pr-4 text-sm text-white placeholder:text-slate-500 outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-500/20" />
            </div>
...
function MenuHeader({ student, walletBalance, rewardsEnabled, points, cartCount, onCart, onLogout }: { student: Student; walletBalance: number; rewardsEnabled: boolean; points: number; cartCount: number; onCart: () => void; onLogout: () => void; }) {
  return <header className="sticky top-0 z-20 border-b border-slate-800 bg-[#131B2E]/95 text-white backdrop-blur-xl">
    <div className={`${CONTAINER} flex min-h-16 items-center justify-between gap-4 py-3`}>
      <div className="flex min-w-0 items-center gap-3">
        <QuickBiteLogo className="h-9 w-9 shrink-0" />
        <div className="min-w-0">
          <p className="text-base font-black leading-none text-white">QuickBite</p>
          <p className="mt-1 truncate text-xs text-slate-400">{student.name}{student.grade ? ` · ${student.grade}` : ''}</p>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-1.5">
        {rewardsEnabled && <span className="hidden rounded-full border border-slate-700 bg-[#0D111D] px-3 py-1.5 text-xs font-bold text-emerald-400 sm:inline-flex">{points} pts</span>}
        <UserNotificationBell userId={student.id} />
        <button onClick={onCart} className="relative rounded-xl border border-slate-800 bg-[#0D111D] p-2 text-slate-200 transition hover:border-slate-700 hover:text-white" aria-label="Abrir carrito"><ShoppingCart className="h-5 w-5" />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-[10px] font-black text-white">{cartCount}</span>}</button>
        <button onClick={onLogout} className="rounded-xl border border-slate-800 bg-[#0D111D] p-2 text-slate-400 transition hover:text-white" aria-label="Cerrar sesión"><LogOut className="h-5 w-5" /></button>
      </div>
    </div>
  </header>;
}
...
function MenuTabs({ tabs, activeTab, onSelect }: { tabs: readonly (readonly [Tab, LucideIcon, string])[]; activeTab: Tab; onSelect: (tab: Tab) => void; }) {
  return <nav className={`${CONTAINER} pt-4`} aria-label="Navegación del menú">
    <div className={`${SURFACE} grid ${tabs.length === 3 ? 'grid-cols-3' : 'grid-cols-2'} rounded-2xl p-1.5`}>
      {tabs.map(([id, Icon, label]) => <button key={id} onClick={() => onSelect(id)} className={`flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold transition ${activeTab === id ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:bg-[#0D111D] hover:text-slate-200'}`}><Icon className="h-4 w-4" />{label}</button>)}
    </div>
  </nav>;
}

function CategoryFilter({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return <button onClick={onClick} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-bold transition ${active ? 'border-emerald-500 bg-emerald-600 text-white' : 'border-slate-800 bg-[#0D111D] text-slate-400 hover:border-slate-700 hover:text-white'}`}>{label}</button>;
}
...
function ProductCard({ product, qty, onAdd, onRemove }: { product: Product; qty: number; onAdd: (product: Product) => void; onRemove: (id: string) => void }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-slate-800 bg-[#0D111D] transition hover:-translate-y-0.5 hover:border-slate-700 hover:shadow-xl hover:shadow-black/20">
    <div className="relative h-44 overflow-hidden bg-[#131B2E]">
      {!imageFailed && product.image_url ? <img src={product.image_url} alt={product.name} className="h-full w-full object-cover" onError={() => setImageFailed(true)} /> : <div className="grid h-full place-items-center text-slate-600"><Utensils className="h-10 w-10" /></div>}
      <span className="absolute left-2 top-2 rounded-full border border-slate-700 bg-[#0D111D]/90 px-2.5 py-1 text-[11px] font-bold text-slate-300 backdrop-blur">Stock {product.stock}</span>
    </div>
    <div className="p-4">
      <p className="min-h-10 text-sm font-black leading-tight text-white">{product.name}</p>
      <p className="mt-1 line-clamp-1 text-xs text-slate-500">{product.description}</p>
      <p className="mt-2 text-lg font-black text-emerald-400">${fmt(product.price)}</p>
      {qty === 0 ? <button onClick={() => onAdd(product)} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl bg-emerald-600 py-2.5 text-sm font-bold text-white transition hover:bg-emerald-500"><Plus className="h-4 w-4" />Agregar</button> : <div className="mt-3 flex items-center justify-between rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-1"><button onClick={() => onRemove(product.id)} className="grid h-8 w-8 place-items-center rounded-lg bg-[#131B2E] text-emerald-400 transition hover:bg-slate-800"><Minus className="h-4 w-4" /></button><span className="font-black text-white">{qty}</span><button onClick={() => onAdd(product)} className="grid h-8 w-8 place-items-center rounded-lg bg-emerald-600 text-white transition hover:bg-emerald-500"><Plus className="h-4 w-4" /></button></div>}
    </div>
  </article>;
}
...
function CartSheet({ cart, cartTotal, total, lastReceipt, payStep, paymentMethod, placing, reference, tip, walletBalance, onAdd, onClose, onPay, onRemove, onSelectPayment, onSetPayStep, onSetTab, onTip }: { cart: CartItem[]; cartTotal: number; total: number; lastReceipt: { orderNumber: string; reference: string; pickup: string; paidWithCredits: boolean } | null; payStep: PayStep; paymentMethod: Order['payment_method']; placing: boolean; reference: string; tip: string; walletBalance: number; onAdd: (product: Product) => void; onClose: () => void; onPay: () => void; onRemove: (id: string) => void; onSelectPayment: (method: Order['payment_method']) => void; onSetPayStep: (step: PayStep) => void; onSetTab: (tab: Tab) => void; onTip: (tip: string) => void; }) {
  ...
  return <div className="fixed inset-0 z-40 flex flex-col justify-end bg-black/70 backdrop-blur-sm" onClick={onClose}>
    <section className="mx-auto max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl border border-slate-800 bg-[#131B2E] text-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-800 bg-[#131B2E]/95 px-5 py-4 backdrop-blur-xl"><h2 className="text-xl font-black">{payStep === 'receipt' ? 'Recibo digital' : payStep === 'payment' ? 'Confirmar pago' : 'Tu pedido'}</h2><button onClick={onClose} className="text-sm font-bold text-slate-400 hover:text-white">Cerrar</button></div>
      <div className="space-y-4 p-5">
        {payStep === 'cart' && <>
          <div className="space-y-3">{cart.map((item) => <div key={item.id} className={`${INNER} flex items-center gap-3 rounded-2xl p-3`}><div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-[#131B2E]">{item.image_url ? <img src={item.image_url} alt={item.name} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-slate-600"><Utensils className="h-5 w-5" /></div>}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{item.name}</p><p className="text-sm font-bold text-emerald-400">${fmt(item.price)} × {item.qty}</p></div><button onClick={() => onRemove(item.id)} className="rounded-xl border border-slate-700 bg-[#131B2E] p-2 text-slate-400 hover:text-white"><Minus className="h-4 w-4" /></button><button onClick={() => onAdd(item)} className="rounded-xl bg-emerald-600 p-2 text-white hover:bg-emerald-500"><Plus className="h-4 w-4" /></button></div>)}</div>
          <div><p className="mb-2 text-sm font-black">Método de pago</p><div className="grid grid-cols-2 gap-2">{paymentOptions.map((opt) => <button key={opt.value} onClick={() => onSelectPayment(opt.value)} className={`rounded-2xl border p-3 text-left transition ${paymentMethod === opt.value ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-800 bg-[#0D111D] hover:border-slate-700'}`}><span className={`mb-2 block h-1.5 w-8 rounded-full ${opt.accent}`} /><span className="block text-sm font-black">{opt.label}</span><span className="text-xs text-slate-500">{opt.hint}</span>{opt.value === 'credits' && <span className={`mt-2 block text-xs font-bold ${creditsInsufficient ? 'text-rose-400' : 'text-violet-400'}`}>Disponible: ${fmt(walletBalance)}</span>}</button>)}</div>{creditsSelected && <div className={`mt-2 rounded-2xl border p-3 text-sm ${creditsInsufficient ? 'border-rose-500/30 bg-rose-500/10 text-rose-300' : 'border-violet-500/30 bg-violet-500/10 text-violet-300'}`}><div className="flex justify-between"><span>Créditos disponibles</span><b>${fmt(walletBalance)}</b></div><div className="mt-1 flex justify-between"><span>Total del pedido</span><b>${fmt(total)}</b></div>{creditsInsufficient ? <p className="mt-2 font-bold">Te faltan ${fmt(total - walletBalance)} para completar esta compra.</p> : <p className="mt-2 font-bold">Al pagar, se descontarán ${fmt(total)} de tu billetera.</p>}</div>}</div>
          <textarea value={tip} onChange={(e) => onTip(e.target.value)} className="w-full rounded-2xl border border-slate-800 bg-[#0D111D] p-3 text-sm text-white placeholder:text-slate-500 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-500/20" rows={2} placeholder="Notas para cafetería" />
          <Summary subtotal={cartTotal} total={total} />
          <Button disabled={creditsInsufficient || cart.length === 0} onClick={() => onSetPayStep('payment')} className="w-full rounded-2xl bg-emerald-600 py-6 text-white hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500">Continuar al pago <ChevronRight className="ml-1 h-5 w-5" /></Button>
        </>}
        {payStep === 'payment' && <><div className={`${INNER} rounded-3xl p-4`}><div className="mb-3 flex items-center gap-2"><CreditCard className="h-5 w-5 text-emerald-400" /><p className="font-black">Pago - {paymentOptions.find((p) => p.value === paymentMethod)?.label}</p></div><p className="text-sm text-slate-400">Referencia: <b className="text-slate-200">{reference}</b></p><p className="text-sm text-slate-400">Total a registrar: <b className="text-slate-200">${fmt(total)}</b></p>{creditsSelected ? <><p className="text-sm text-slate-400">Créditos disponibles: <b className="text-slate-200">${fmt(walletBalance)}</b></p><p className="mt-4 rounded-2xl border border-violet-500/20 bg-violet-500/10 p-3 text-sm text-violet-300">Al confirmar, QuickBite descontará el total de tu saldo de créditos de forma segura y atómica. El pedido quedará pagado inmediatamente.</p></> : <p className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-300">El pago quedará pendiente hasta que un administrador lo revise y apruebe. El QR de recogida se habilitará únicamente después de la aprobación.</p>}</div><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => onSetPayStep('cart')} className="rounded-2xl border-slate-700 bg-transparent py-6 text-slate-200 hover:bg-slate-800">Volver</Button><Button disabled={placing || creditsInsufficient} onClick={onPay} className="rounded-2xl bg-emerald-600 py-6 text-white hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500">{placing ? 'Procesando...' : creditsSelected ? 'Pagar con créditos' : 'Enviar para aprobación'}</Button></div></>}
        {payStep === 'receipt' && lastReceipt && <div className={`rounded-3xl border border-dashed p-5 text-center ${lastReceipt.paidWithCredits ? 'border-violet-500/40 bg-violet-500/10' : 'border-amber-500/40 bg-amber-500/10'}`}><Clock3 className={`mx-auto mb-3 h-12 w-12 ${lastReceipt.paidWithCredits ? 'text-violet-400' : 'text-amber-400'}`} /><p className="text-sm font-bold text-slate-400">Pedido enviado</p><p className="text-3xl font-black text-white">{lastReceipt.orderNumber}</p><p className={`mt-3 text-sm font-bold ${lastReceipt.paidWithCredits ? 'text-violet-300' : 'text-amber-300'}`}>{lastReceipt.paidWithCredits ? 'Pago con créditos confirmado' : 'Pago pendiente de aprobación'}</p><p className="mt-2 text-sm text-slate-400">{lastReceipt.paidWithCredits ? 'Se descontaron los créditos del pedido. Tu QR de recogida ya está disponible en Historial.' : 'El código QR y el código de recogida estarán disponibles cuando Admin apruebe el pago.'}</p><p className="mt-3 text-xs text-slate-500">Ref. pago: {lastReceipt.reference}</p><Button onClick={() => { onClose(); onSetTab('orders'); onSetPayStep('cart'); }} className="mt-5 w-full rounded-2xl bg-emerald-600 py-6 text-white hover:bg-emerald-500">Ver historial</Button></div>}
      </div>
    </section>
  </div>;
}

function Summary({ subtotal, total }: { subtotal: number; total: number }) { return <div className={`${INNER} space-y-2 rounded-3xl p-4 text-sm`}><div className="flex justify-between border-t border-slate-800 pt-2 text-lg font-black"><span>Total</span><span className="text-emerald-400">${fmt(total)}</span></div>{subtotal !== total && <div className="text-xs text-slate-500">Subtotal: ${fmt(subtotal)}</div>}</div>; }
function Empty({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }) { return <div className={`${SURFACE} mt-4 rounded-3xl p-8 text-center`}><Icon className="mx-auto mb-3 h-12 w-12 text-emerald-500/50" /><h3 className="font-black text-white">{title}</h3><p className="mt-1 text-sm text-slate-400">{text}</p></div>; }
...
function OrderCard({ order }: { order: Order }) {
  ...
  return <article className={`${SURFACE} overflow-hidden rounded-3xl`}><button onClick={() => setExpanded((value) => !value)} className="w-full p-4 text-left"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-slate-500">#{order.order_number}</p><h3 className="font-black text-white">{label}</h3><p className="mt-1 text-xs text-slate-500">{created.toLocaleDateString('es-CO', { day: '2-digit', month: 'long', year: 'numeric' })} · {created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</p></div><div className="flex items-center gap-2"><Badge className={paymentApproved ? 'bg-emerald-600 text-white' : order.payment_status === 'rejected' ? 'bg-red-600 text-white' : 'bg-amber-500 text-white'}>{paymentApproved ? 'Pagado' : order.payment_status === 'rejected' ? 'Rechazado' : 'Pendiente'}</Badge><ChevronDown className={`h-5 w-5 text-slate-500 transition-transform ${expanded ? 'rotate-180' : ''}`} /></div></div><div className="mt-3 flex items-center gap-3"><div className={`grid h-10 w-10 place-items-center rounded-2xl ${cancelled ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}><Icon className="h-5 w-5" /></div><div className="flex-1"><div className="h-2 overflow-hidden rounded-full bg-slate-800"><div className={`h-full rounded-full ${cancelled ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${progress}%` }} /></div><p className="mt-1 text-xs text-slate-500">{cancelled ? 'El pago fue rechazado y el pedido quedó cancelado.' : <>Recogida: <b className="text-slate-300">{paymentApproved ? (order.pickup_code ?? '----') : 'Disponible tras aprobación'}</b> - {order.estimated_minutes ?? 12} min</>}</p></div></div><div className="mt-3 flex justify-between border-t border-slate-800 pt-3 text-sm"><span className="text-slate-500">Total</span><b className="text-emerald-400">${fmt(order.total)}</b></div></button>{expanded && <div className="border-t border-slate-800 bg-[#0D111D] p-4"><div className="space-y-2">{order.order_items?.length ? order.order_items.map((item) => <div key={item.id} className={`${INNER} flex items-center justify-between gap-3 rounded-2xl px-3 py-2`}><div className="min-w-0"><p className="truncate text-sm font-bold text-white">{item.product?.name ?? 'Producto'}</p><p className="text-xs text-slate-500">{item.quantity} × ${fmt(Number(item.price))}</p></div><b className="text-sm text-emerald-400">${fmt(Number(item.price) * item.quantity)}</b></div>) : <p className="text-xs text-slate-500">No hay detalle de artículos disponible.</p>}</div><div className={`${INNER} mt-3 grid gap-2 rounded-2xl p-3 text-sm`}><div className="flex justify-between"><span className="text-slate-500">Fecha</span><b>{created.toLocaleDateString('es-CO')}</b></div><div className="flex justify-between"><span className="text-slate-500">Hora</span><b>{created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</b></div><div className="flex justify-between"><span className="text-slate-500">Método de pago</span><b>{order.payment_method === 'bre-b' ? 'Bre-B' : order.payment_method === 'cash' ? 'Efectivo' : order.payment_method === 'credits' ? 'Créditos' : 'Nequi'}</b></div>{order.notes && <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-2"><p className="text-xs font-bold text-amber-300">Comentario</p><p className="text-sm text-slate-300">{order.notes}</p></div>}</div>{paymentApproved ? <div className="mt-3 flex flex-col items-center rounded-3xl border border-dashed border-emerald-500/30 bg-emerald-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wider text-emerald-300">Código QR para reclamar</p><QRCodeSVG value={getOrderVerificationUrl(order.pickup_code ?? order.order_number)} size={170} level="M" includeMargin className="mt-3 rounded-xl bg-white p-2" /><p className="mt-2 text-2xl font-black tracking-[0.2em] text-emerald-400">{order.pickup_code ?? '----'}</p><p className="mt-1 text-xs text-slate-500">Escanea este QR para verificar el pedido o preséntalo al recogerlo.</p></div> : <div className={`mt-3 rounded-3xl border border-dashed p-4 text-center ${cancelled ? 'border-red-500/30 bg-red-500/10' : 'border-amber-500/30 bg-amber-500/10'}`}><Clock3 className={`mx-auto h-8 w-8 ${cancelled ? 'text-red-400' : 'text-amber-400'}`} /><p className={`mt-2 text-sm font-bold ${cancelled ? 'text-red-300' : 'text-amber-300'}`}>{cancelled ? 'Pedido cancelado' : 'QR bloqueado'}</p><p className="mt-1 text-xs text-slate-500">{cancelled ? 'El pago fue rechazado; el pedido no será preparado ni entregado.' : 'El QR aparecerá cuando Admin apruebe el pago.'}</p></div>}</div>}</article>;
}
...
function RedemptionCard({ redemption }: { redemption: LoyaltyRedemption }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-slate-800 bg-[#131B2E]"><button onClick={() => setExpanded((value) => !value)} className="w-full p-4 text-left"><div className="flex items-start justify-between gap-3"><div><Badge className="bg-emerald-500/10 text-emerald-300">CANJE</Badge><h3 className="mt-2 font-black text-white">{productName}</h3><p className="mt-1 text-xs text-slate-500">{created.toLocaleDateString('es-CO')} · {created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</p></div><div className="flex items-center gap-2"><Badge className={statusClass}>{statusLabel}</Badge><ChevronDown className={`h-5 w-5 text-slate-500 transition-transform ${expanded ? 'rotate-180' : ''}`} /></div></div><div className="mt-3 flex justify-between rounded-2xl border border-slate-800 bg-[#0D111D] px-3 py-2 text-sm"><span className="text-slate-500">Puntos canjeados</span><b className="text-emerald-400">{redemption.points_spent} pts</b></div></button>{expanded && <div className="border-t border-slate-800 bg-[#0D111D] p-4"><div className="grid gap-2 rounded-2xl border border-slate-800 bg-[#131B2E] p-3 text-sm"><div className="flex justify-between"><span className="text-slate-500">Tipo</span><b>Canje</b></div><div className="flex justify-between"><span className="text-slate-500">Producto</span><b>{productName}</b></div><div className="flex justify-between"><span className="text-slate-500">Puntos</span><b>{redemption.points_spent}</b></div><div className="flex justify-between"><span className="text-slate-500">Fecha</span><b>{created.toLocaleDateString('es-CO')}</b></div><div className="flex justify-between"><span className="text-slate-500">Hora</span><b>{created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</b></div><div className="flex justify-between"><span className="text-slate-500">Estado</span><b>{statusLabel}</b></div></div>{approved ? <div className="mt-3 flex flex-col items-center rounded-3xl border border-dashed border-emerald-500/30 bg-emerald-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wider text-emerald-300">QR para reclamar canje</p><QRCodeSVG value={redemption.redemption_code} size={170} className="mt-3 rounded-xl bg-white p-2" /><p className="mt-2 text-xl font-black tracking-[0.12em] text-emerald-400">{redemption.redemption_code}</p></div> : <div className="mt-3 rounded-3xl border border-dashed border-amber-500/30 bg-amber-500/10 p-4 text-center"><Clock3 className="mx-auto h-8 w-8 text-amber-400" /><p className="mt-2 text-sm font-bold text-amber-300">QR bloqueado</p><p className="mt-1 text-xs text-slate-500">El QR aparecerá únicamente cuando Admin apruebe el canje.</p></div>}</div>}</article>;
}
