...
export function StudentMenuPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-900" data-pickup-slots={futurePickupSlots.length}>
...
function statusData(order: Order): [string, number, LucideIcon] { if (order.status === 'pending') return ['Pendiente', 0, Clock3]; if (order.status === 'preparing') return ['Preparando', 1, Utensils]; if (order.status === 'ready') return ['Listo', 2, PackageCheck]; if (order.status === 'delivered') return ['Entregado', 3, CheckCircle2]; return ['Cancelado', 0, ReceiptText]; }
function OrderCard({ order }: { order: Order }) { const [label, step, Icon] = statusData(order); return <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold text-slate-500">{order.order_number}</p><p className="text-lg font-black">${fmt(order.total)}</p></div><Badge>{label}</Badge></div><div className="mt-4 flex items-center gap-2 text-sm text-slate-600"><Icon className="h-4 w-4 text-green-600" />Estado actual: {label}</div><div className="mt-4 flex items-center gap-1">{[0,1,2,3].map((value) => <div key={value} className={`h-1.5 flex-1 rounded-full ${value <= step ? 'bg-green-600' : 'bg-slate-200'}`} />)}</div></article>; }
