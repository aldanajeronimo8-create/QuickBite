...
export function StudentMenuPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-900">
        ...
        <div className="mx-auto max-w-6xl px-5 pb-5 pt-5 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowCart(true)} className="relative rounded-full bg-white/10 p-2" aria-label="Abrir carrito"><ShoppingCart className="h-5 w-5" />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-green-500 text-xs font-black">{cartCount}</span>}</button>
          ...
          <div className="mt-5 rounded-[2rem] bg-gradient-to-r from-green-600 to-green-700 p-4 text-white shadow-lg shadow-green-950/10">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-green-100">Recreo inteligente</p><h1 className="mt-1 text-2xl font-black">Pide ahora, recoge sin fila</h1></div>{rewardsEnabled && <Badge className="bg-white text-green-800">{loyalty.availablePoints} pts</Badge>}</div>
      ...
      <nav className={`sticky top-[190px] z-10 mx-5 mt-5 grid ${rewardsEnabled ? 'grid-cols-3' : 'grid-cols-2'} rounded-3xl bg-white p-1.5 shadow-lg ring-1 ring-slate-200`}>
        {navigationTabs.map(([id, Icon, label]) => (
          <button key={id} onClick={() => setTab(id)} className={`flex items-center justify-center gap-1 rounded-xl py-2 text-sm font-bold transition ${activeTab === id ? 'bg-green-600 text-white shadow-sm' : 'text-slate-600 hover:bg-green-50'}`}><Icon className="h-4 w-4" />{label}</button>
        ))}
      </nav>
    ...
    <div className="fixed inset-0 z-40 flex flex-col justify-end bg-slate-950/50" onClick={onClose}>
      <section className="max-h-[88vh] overflow-y-auto rounded-t-[2rem] bg-white" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 p-5">
          {payStep === 'cart' && <>
            <div className="space-y-3">{cart.map((item) => <div key={item.id} className="flex items-center gap-3"><img src={item.image_url} alt={item.name} className="h-14 w-14 rounded-2xl object-cover" /><div className="flex-1"><p className="text-sm font-black">{item.name}</p><p className="text-sm font-bold text-green-800">${fmt(item.price)} x {item.qty}</p></div><button onClick={() => onRemove(item.id)} className="rounded-full bg-green-50 p-2 text-green-700"><Minus className="h-4 w-4" /></button><button onClick={() => onAdd(item)} className="rounded-full bg-green-600 p-2 text-white hover:bg-green-700"><Plus className="h-4 w-4" /></button></div>)}</div>
            <div><p className="mb-2 text-sm font-black">Método de pago</p><div className="grid grid-cols-2 gap-2">{paymentOptions.map((opt) => <button key={opt.value} onClick={() => onSelectPayment(opt.value)} className={`rounded-2xl border p-3 text-left ${paymentMethod === opt.value ? 'border-green-600 bg-green-50' : 'border-slate-200'}`}><span className={`mb-2 block h-2 w-8 rounded-full ${opt.accent}`} /><span className="block text-sm font-black">{opt.label}</span><span className="text-xs text-slate-600">{opt.hint}</span></button>)}</div></div>
            <textarea value={tip} onChange={(e) => onTip(e.target.value)} className="w-full rounded-2xl border border-slate-200 p-3 text-sm outline-none focus:ring-2 focus:ring-green-200" rows={2} placeholder="Notas para cafeteria" />
            <Summary subtotal={cartTotal} fee={fee} total={total} />
            <Button onClick={() => onSetPayStep('payment')} className="w-full rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700">Continuar al pago <ChevronRight className="ml-1 h-5 w-5" /></Button>
          </>}
          {payStep === 'payment' && <>
            <div className="rounded-3xl bg-slate-50 p-4">
              <div className="mb-3 flex items-center gap-2"><CreditCard className="h-5 w-5 text-green-600" /><p className="font-black">Pago - {paymentOptions.find((p) => p.value === paymentMethod)?.label}</p></div>
            ...
            <div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => onSetPayStep('cart')} className="rounded-2xl py-6">Volver</Button><Button disabled={placing} onClick={onPay} className="rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700">{placing ? 'Procesando...' : 'Pagar'}</Button></div>
          </>}
          {payStep === 'receipt' && lastReceipt && <div className="rounded-3xl border border-dashed border-green-300 bg-green-50 p-5 text-center"><CheckCircle2 className="mx-auto mb-3 h-12 w-12 text-green-600" /><p className="text-sm font-bold text-slate-600">Pedido creado</p><p className="text-3xl font-black text-slate-900">{lastReceipt.orderNumber}</p><p className="mt-3 text-sm">Código de recogida</p><p className="text-4xl font-black tracking-[0.2em] text-green-800">{lastReceipt.pickup}</p><p className="mt-3 text-xs text-slate-600">Ref. pago: {lastReceipt.reference}</p><Button onClick={() => { onClose(); onSetTab('orders'); onSetPayStep('cart'); }} className="mt-5 w-full rounded-2xl bg-green-600 py-6 text-white hover:bg-green-700">Ver historial</Button></div>}
        </div>
      </section>
    </div>
  );
