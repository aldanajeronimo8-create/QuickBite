...
function OrderCard({ order }: { order: Order }) { const [label, progress, Icon] = statusData(order); return <article className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-xs font-bold text-slate-400">#{order.order_number}</p><h3 className="font-black">{label}</h3><p className="mt-1 text-xs text-slate-500">{new Date(order.created_at).toLocaleDateString('es-CO', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p></div><Badge className={order.payment_status === 'confirmed' ? 'bg-green-500 text-white' : order.payment_status === 'rejected' ? 'bg-red-600 text-white' : 'bg-amber-500 text-white'}>{order.payment_status === 'confirmed' ? 'Pagado' : order.payment_status === 'rejected' ? 'Rechazado' : 'Pendiente'}</Badge></div><div className="mt-3 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-green-50 text-green-700"><Icon className="h-5 w-5" /></div><div className="flex-1"><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-green-600" style={{ width: `${progress}%` }} /></div><p className="mt-1 text-xs text-slate-600">Recogida: <b>{order.pickup_code ?? '----'}</b> - {order.estimated_minutes ?? 12} min</p></div></div><div className="mt-4 border-t pt-4"><p className="mb-2 text-sm font-black text-slate-800">Detalle de la compra</p><div className="space-y-2">{order.order_items?.length ? order.order_items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2"><div className="min-w-0"><p className="truncate text-sm font-bold">{item.product?.name ?? 'Producto'}</p><p className="text-xs text-slate-500">{item.quantity} × ${fmt(Number(item.price))}</p></div><b className="text-sm text-green-800">${fmt(Number(item.price) * item.quantity)}</b></div>) : <p className="text-xs text-slate-500">No hay detalle de artículos disponible.</p>}</div>{order.notes && <div className="mt-3 rounded-2xl border border-amber-100 bg-amber-50 p-3"><p className="text-xs font-bold text-amber-800">Comentario</p><p className="mt-1 text-sm text-slate-700">{order.notes}</p></div>}<div className="mt-3 flex justify-between border-t pt-3 text-sm"><span className="text-slate-600">Total pagado</span><b className="text-lg text-green-800">${fmt(order.total)}</b></div></div></article>; }
