...
import { CheckCircle2, ChevronDown, ChevronRight, Clock3, CreditCard, History, Home, LogOut, Minus, PackageCheck, Plus, ReceiptText, Search, ShoppingCart, Star, Utensils } from 'lucide-react';
...
export function StudentMenuPage() {
  ...
  const handleRewardRedemption = async (reward: LoyaltyReward) => {
    ...
    try {
      ...
      toast.success(`Canje solicitado. Presenta el codigo ${redemption.redemption_code} en caja cuando sea aprobado.`);
...
function OrderCard({ order }: { order: Order }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><button onClick={() => setExpanded((value) => !value)} className="w-full p-4 text-left"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-slate-400">#{order.order_number}</p><h3 className="font-black">{label}</h3><p className="mt-1 text-xs text-slate-500">{created.toLocaleDateString('es-CO', { day: '2-digit', month: 'long', year: 'numeric' })} · {created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</p></div><div className="flex items-center gap-2"><Badge className={order.payment_status === 'confirmed' ? 'bg-green-500 text-white' : order.payment_status === 'rejected' ? 'bg-red-600 text-white' : 'bg-amber-500 text-white'}>{order.payment_status === 'confirmed' ? 'Pagado' : order.payment_status === 'rejected' ? 'Rechazado' : 'Pendiente'}</Badge><ChevronDown className={`h-5 w-5 text-slate-400 transition-transform ${expanded ? 'rotate-180' : ''}`} /></div></div><div className="mt-3 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-green-50 text-green-700"><Icon className="h-5 w-5" /></div><div className="flex-1"><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-green-600" style={{ width: `${progress}%` }} /></div><p className="mt-1 text-xs text-slate-600">Recogida: <b>{order.pickup_code ?? '----'}</b> - {order.estimated_minutes ?? 12} min</p></div></div><div className="mt-3 flex justify-between border-t pt-3 text-sm"><span className="text-slate-600">Total</span><b className="text-green-800">${fmt(order.total)}</b></div></button>
    {expanded && <div className="border-t bg-slate-50/70 p-4"><div className="space-y-2">{order.order_items?.length ? order.order_items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl bg-white px-3 py-2 ring-1 ring-slate-200"><div className="min-w-0"><p className="truncate text-sm font-bold">{item.product?.name ?? 'Producto'}</p><p className="text-xs text-slate-500">{item.quantity} × ${fmt(Number(item.price))}</p></div><b className="text-sm text-green-800">${fmt(Number(item.price) * item.quantity)}</b></div>) : <p className="text-xs text-slate-500">No hay detalle de artículos disponible.</p>}</div><div className="mt-3 grid gap-2 rounded-2xl bg-white p-3 text-sm ring-1 ring-slate-200"><div className="flex justify-between"><span className="text-slate-500">Fecha</span><b>{created.toLocaleDateString('es-CO')}</b></div><div className="flex justify-between"><span className="text-slate-500">Hora</span><b>{created.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}</b></div><div className="flex justify-between"><span className="text-slate-500">Método de pago</span><b>{order.payment_method}</b></div>{order.notes && <div className="rounded-xl bg-amber-50 p-2"><p className="text-xs font-bold text-amber-800">Comentario</p><p className="text-sm text-slate-700">{order.notes}</p></div>}</div><div className="mt-3 flex flex-col items-center rounded-3xl border border-dashed border-green-300 bg-green-50 p-4"><p className="text-xs font-bold uppercase tracking-wider text-green-800">Código QR para reclamar</p><QRCodeSVG value={order.pickup_code ?? order.order_number} size={170} className="mt-3 rounded-xl bg-white p-2" /><p className="mt-2 text-2xl font-black tracking-[0.2em] text-green-800">{order.pickup_code ?? '----'}</p><p className="mt-1 text-xs text-slate-500">Presenta este código al recoger tu pedido.</p></div></div>}
  </article>;
}
