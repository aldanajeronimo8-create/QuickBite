...
export function StudentRegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', confirmPassword: '', ti: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.ti.trim())
      errs.ti = 'Ingresa tu T.I.';
    else if (!/^\d+$/.test(form.ti.trim()))
      errs.ti = 'Solo se permiten números';
    else if (form.ti.trim().length > 11)
      errs.ti = 'Máximo 11 dígitos';
      ...
      if (error) {
        ...
        if (/already registered|already exists/i.test(msg)) {
          ...
          if (loginError || !loginData.user) {
            setErrors({ email: 'Este correo ya tiene una cuenta. Usa la misma contraseña de esa cuenta para añadir el perfil de estudiante.' });
            return;
          }
          ...
          if (rpcErr) {
            ...
            if (/already|duplicate/i.test(rpcErr.message) && /student|profile/i.test(rpcErr.message)) {
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              {errors.ti
                ...
                : <p className="text-gray-400 text-xs mt-1">Solo números · máximo 11 dígitos · única por estudiante</p>
              }
            </div>
