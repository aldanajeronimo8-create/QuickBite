...
export function StudentRegisterPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', confirmPassword: '', ti: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const metadata = {
      ...
      role: 'student',
      data_consent_version: PRIVACY_VERSION,
    ...
    try {
      ...
      if (error && /already registered|already exists/i.test(error.message)) {
        ...
        if (loginError || !loginData.user) {
          setErrors({ email: 'Este correo ya tiene una cuenta. Usa la contraseña correspondiente o recupera el acceso.' });
          return;
        }
      ...
      if (!userId) throw new Error('No fue posible crear la cuenta.');
      ...
      const { error: rpcError } = await supabase.rpc('create_student_profile_with_consent', {
        ...
        p_guardian_relationship: form.guardianRelationship.trim(),
        p_guardian_email: form.guardianEmail.trim().toLowerCase(),
        p_student_acknowledged: true,
        p_guardian_authorized: true,
        p_purpose: DATA_PURPOSE,
      });

      if (rpcError) {
        if (/ti_already_registered|duplicate/i.test(rpcError.message) && /ti/i.test(rpcError.message)) {
          setErrors({ ti: 'Esta T.I. ya está registrada.' });
        } else if (/consent|authorization|guardian/i.test(rpcError.message)) {
          setErrors({ guardianAuthorized: 'No se pudo registrar la autorización de datos. Revisa los datos del representante.' });
        } else {
          throw rpcError;
        }
        return;
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 flex flex-col items-center justify-center p-5">
      ...
      <div className="w-full max-w-lg relative z-10">
        <div className="text-center mb-6">
          <QuickBiteLogo className="mx-auto mb-3 h-16 w-16 rounded-3xl" />
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-7 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <section className="space-y-3 rounded-2xl border border-slate-200 p-4">
              <h3 className="font-black text-slate-800">1. Datos del estudiante</h3>
            ...
            <section className="space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
              <h3 className="font-black text-slate-800">2. Representante legal o tutor</h3>
              <p className="text-xs text-slate-600">Para estudiantes menores de edad, la autorización del representante legal/tutor es un requisito del tratamiento de datos personales.</p>
            ...
            <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3">
              <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-green-700" /><div><p className="font-black text-slate-800">3. Hábeas data y autorización</p><p className="mt-1 text-xs leading-5 text-slate-600">QuickBite solicita autorización previa, expresa e informada para tratar los datos necesarios para la cuenta, pedidos, pagos, historial, puntos, recompensas y atención de solicitudes sobre datos personales.</p></div></div>
              ...
              {showPrivacy && <div className="rounded-xl border border-slate-200 bg-white p-3 text-xs leading-5 text-slate-600"><p><b>Finalidad:</b> {DATA_PURPOSE}</p><p className="mt-2"><b>Datos tratados:</b> nombre, correo, T.I., grado/curso, pedidos/compras, pagos, puntos y recompensas y comunicaciones operativas.</p><p className="mt-2"><b>Derechos:</b> conocer, actualizar, rectificar y acceder a los datos; solicitar información sobre su uso; y ejercer los mecanismos de consulta, reclamo, revocatoria o supresión cuando legalmente proceda.</p><p className="mt-2"><b>Seguridad:</b> el acceso debe limitarse a usuarios autorizados y los datos se conservarán bajo medidas de seguridad apropiadas.</p><p className="mt-2"><b>Versión:</b> {PRIVACY_VERSION}. Esta pantalla es un registro de autorización para el prototipo; el Colegio/Responsable debe publicar su política integral y canales oficiales de atención.</p></div>}
              <label className="flex items-start gap-3 text-sm text-slate-700"><input type="checkbox" checked={studentAcknowledged} onChange={(e) => setStudentAcknowledged(e.target.checked)} className="mt-1 h-4 w-4 accent-green-600" /><span>He leído el aviso de privacidad y comprendo para qué se tratarán mis datos. <b>Este consentimiento queda registrado.</b></span></label>
              {errors.studentAcknowledged && <p className="text-xs text-red-500">{errors.studentAcknowledged}</p>}
              <label className="flex items-start gap-3 text-sm text-slate-700"><input type="checkbox" checked={guardianAuthorized} onChange={(e) => setGuardianAuthorized(e.target.checked)} className="mt-1 h-4 w-4 accent-green-600" /><span>Declaro que el representante legal o tutor identificado arriba autoriza el tratamiento de los datos del estudiante para las finalidades informadas y ha podido conocer esta autorización.</span></label>
              {errors.guardianAuthorized && <p className="text-xs text-red-500">{errors.guardianAuthorized}</p>}
...
function PasswordField({ label, value, error, visible, onToggle, onChange }: { label: string; value: string; error?: string; visible: boolean; onToggle: () => void; onChange: (value: string) => void }) {
  return <div><Label className="text-gray-700 text-sm mb-1 block">{label}</Label><div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input type={visible ? 'text' : 'password'} placeholder="Mínimo 6 caracteres" value={value} onChange={(e) => onChange(e.target.value)} className={`pl-9 pr-10 ${error ? 'border-red-400' : ''}`} /><button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>{error && <p className="text-red-500 text-xs mt-1">{error}</p>}</div>;
}
