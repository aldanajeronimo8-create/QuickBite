...
type CancellationTarget = { orderId: string; itemId: string; productName: string; quantity: number };
...
export function StudentHistoryPage() {
  ...
  const requestProductCancellation = async () => {
    ...
    setBusy(true);
    try {
      const { error } = await requireSupabaseClient().rpc('request_order_item_cancellation', { p_order_item_id: cancelling.itemId, p_reason: reason.trim() });
      if (error) throw error;
      toast.success(`Solicitud enviada para ${cancelling.productName}.`);
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(22,163,106,.14),_transparent_38%),#f5f8f7] p-5 sm:p-8 text-slate-900">
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {cancelling && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-5"><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"><p className="text-xs font-black uppercase tracking-wide text-red-700">Solicitud de cancelación de producto</p><h2 className="mt-1 text-xl font-black">{cancelling.productName}</h2><p className="mt-1 text-sm text-slate-600">Cantidad: {cancelling.quantity}. La solicitud será revisada por administración. Si corresponde un reembolso, quedará registrado en el historial financiero.</p><textarea value={reason} onChange={(e) => setReason(e.target.value)} maxLength={500} rows={4} placeholder="Indica el motivo..." className="mt-4 w-full rounded-2xl border border-slate-200 p-3 text-sm outline-none focus:border-emerald-500"/><div className="mt-4 flex gap-2"><button type="button" onClick={() => { setCancelling(null); setReason(''); }} className="flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black text-slate-700">Cerrar</button><button type="button" disabled={busy} onClick={() => void requestProductCancellation()} className="flex-1 rounded-2xl bg-red-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{busy ? 'Enviando...' : 'Enviar solicitud'}</button></div></div></div>}
    </div>
  </div>;
}
