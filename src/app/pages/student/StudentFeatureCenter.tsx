  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(22,163,106,.14),_transparent_38%),radial-gradient(circle_at_top_right,_rgba(37,99,235,.12),_transparent_34%),#f5f8f7] p-5 text-slate-900 sm:p-8">
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Link to="/menu?tab=menu" className="rounded-3xl border border-white/60 bg-white/65 p-5 shadow-sm backdrop-blur-xl hover:-translate-y-0.5 hover:shadow-lg"><UtensilsCrossed className="h-5 w-5 text-emerald-700"/><h2 className="mt-3 font-black">Menú</h2><p className="mt-1 text-sm text-slate-600">Comprar y consultar disponibilidad.</p></Link>
        <Link to="/student/history" className="rounded-3xl border border-white/60 bg-white/65 p-5 shadow-sm backdrop-blur-xl hover:-translate-y-0.5 hover:shadow-lg"><History className="h-5 w-5 text-emerald-700"/><h2 className="mt-3 font-black">Historial</h2><p className="mt-1 text-sm text-slate-600">Ver pedidos, estados y recibos.</p></Link>
        <Link to="/student/rewards" className="rounded-3xl border border-white/60 bg-white/65 p-5 shadow-sm backdrop-blur-xl hover:-translate-y-0.5 hover:shadow-lg"><Star className="h-5 w-5 text-emerald-700"/><h2 className="mt-3 font-black">Puntos</h2><p className="mt-1 text-sm text-slate-600">Consultar y canjear recompensas.</p></Link>
        <Link to="/student/notifications" className="rounded-3xl border border-white/60 bg-white/65 p-5 shadow-sm backdrop-blur-xl hover:-translate-y-0.5 hover:shadow-lg"><Bell className="h-5 w-5 text-emerald-700"/><h2 className="mt-3 font-black">Notificaciones</h2><p className="mt-1 text-sm text-slate-600">Abrir avisos y cambios del pedido.</p></Link>
      </div>
      <section id="favorites" className="rounded-[2rem] border border-white/60 bg-white/70 p-6 shadow-xl backdrop-blur-2xl"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Heart className="h-6 w-6 text-rose-500"/><div><h2 className="font-black">Favoritos</h2><p className="text-sm text-slate-600">Se guardan en tu cuenta de QuickBite.</p></div></div><Link to="/student/features#favorites" className="rounded-full border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700">Ver favoritos</Link></div>{loading ? <p className="mt-5 text-sm text-slate-500">Cargando favoritos…</p> : favorites.length === 0 ? <p className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">Aún no tienes favoritos. Elige productos del menú para guardarlos.</p> : <div className="mt-5 grid gap-4 sm:grid-cols-2">{favorites.map((product) => <div key={product.id} className="flex gap-3 rounded-2xl border border-white/60 bg-white/60 p-3"><img src={product.image_url} alt={product.name} className="h-20 w-20 rounded-xl object-cover"/><div className="min-w-0 flex-1"><p className="font-black">{product.name}</p><p className="text-sm text-emerald-700">${product.price.toLocaleString('es-CO')}</p><button type="button" onClick={() => void toggleFavorite(product.id)} disabled={saving === product.id} className="mt-2 text-xs font-bold text-rose-600">Quitar de favoritos</button></div></div>)}</div>}</section>
