...
function getLinkErrorMessage(error: unknown) {
  ...
  return message || 'No se pudo vincular el estudiante.';
}
...
export function ParentFamilyPage() {
  ...
  useEffect(() => { void load().catch((error) => toast.error(error instanceof Error ? getLinkErrorMessage(error) : 'No se pudieron cargar los estudiantes vinculados.')).finally(() => setLoading(false)); }, [load]);
  ...
  const enterStudent = (student: ActingStudent) => {
