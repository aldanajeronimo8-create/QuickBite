      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700 dark:text-emerald-300">QuickBite Family</p>
              <h1 className="qb-text mt-1 text-3xl font-black">Bienestar de {activeStudent.full_name}</h1>
              <p className="qb-text-secondary mt-1 text-sm">Resumen de consumo, límites y actividad reciente del estudiante.</p>
            </div>
          </div>
        ...
        <section className="qb-surface rounded-[2rem] border qb-border p-6 shadow-lg">
          <div className="flex items-start gap-3"><WalletCards className="mt-1 h-6 w-6 text-blue-600"/><div><h2 className="qb-text text-xl font-black">Control de gasto</h2><p className="qb-text-secondary mt-1 text-sm">Consulta el consumo frente a los límites configurados para el estudiante.</p></div></div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {[
              ['Diario', dailySpent, dailyLimit],
              ['Semanal', summary.week, weeklyLimit],
              ['Mensual', monthlySpent, monthlyLimit],
            ].map(([label, value, limit]) => {
              const numericValue = Number(value ?? 0);
              const numericLimit = limit == null ? null : Number(limit);
              return <article key={String(label)} className="rounded-2xl bg-[var(--qb-surface-muted)] p-4"><div className="flex items-center justify-between"><span className="qb-text font-bold">{label}</span><span className="qb-text-secondary text-xs">{numericLimit == null ? 'Sin límite' : money(numericLimit)}</span></div><p className="qb-text mt-2 text-lg font-black">{money(numericValue)}</p>{numericLimit != null && <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800"><div className="h-full rounded-full bg-blue-500" style={{ width: `${percentage(numericValue, numericLimit)}%` }} /></div>}</article>;
            })}
          </div>
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="qb-text-secondary text-xs">Los límites configurados se mantienen vigentes al registrar nuevos pedidos.</p><Button type="button" onClick={() => { const target = document.getElementById('spending-limits'); target?.scrollIntoView({ behavior: 'smooth' }); }} variant="outline">Administrar límites</Button></div>
        </section>

        <section id="spending-limits" className="qb-surface rounded-[2rem] border qb-border p-6 shadow-lg">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-3">
            {([['daily_limit','Diario'],['weekly_limit','Semanal'],['monthly_limit','Mensual']] as const).map(([key,label]) => <label key={key} className="space-y-2"><span className="qb-text text-sm font-bold qb-text">{label}</span><Input type="number" min="0" step="500" value={limitValue(limits[key])} onChange={(e) => setLimits((current) => ({ ...current, [key]: e.target.value === '' ? null : Number(e.target.value) }))} placeholder="Sin límite" /></label>)}
          </div>
