...
export function AdminReviews() {
  ...
  return <div className="space-y-6 qb-admin-reviews">
    ...
    <section className="space-y-3">
      ...
      {filtered.map((review) => <article key={review.id} className="qb-surface rounded-3xl border qb-border p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0 flex-1"><div className="flex items-start gap-3">{review.product?.image_url ? <img src={review.product.image_url} alt="" className="h-14 w-14 rounded-2xl object-cover"/> : <div className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-100 dark:bg-slate-800">🍽️</div>}<div><h2 className="qb-text font-black">{review.product?.name ?? 'Producto'}</h2><p className="qb-text-secondary text-xs">{review.student?.full_name ?? 'Estudiante'} · {review.student?.email ?? 'sin correo'}</p><div className="mt-2 flex items-center gap-2"><Stars value={review.stars}/><span className="qb-text-secondary text-xs">{new Date(review.created_at).toLocaleDateString('es-CO')}</span></div></div></div>{review.comment && <p className="admin-review-comment qb-text-secondary mt-4 rounded-2xl border p-4 text-sm">“{review.comment}”</p>}</div><div className="flex shrink-0 flex-col gap-2 sm:flex-row lg:flex-col"><span className={`rounded-full px-3 py-1.5 text-center text-xs font-black ${review.status === 'approved' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-400/10 dark:text-emerald-300' : review.status === 'rejected' ? 'bg-rose-100 text-rose-800 dark:bg-rose-400/10 dark:text-rose-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-400/10 dark:text-amber-300'}`}>{statusLabel[review.status]}</span>{review.status === 'pending' && <><button type="button" disabled={savingId === review.id} onClick={() => void moderate(review, 'approved')} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><Check className="h-4 w-4"/>Publicar</button><button type="button" disabled={savingId === review.id} onClick={() => void moderate(review, 'rejected')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-700 disabled:opacity-50 dark:border-rose-300/20 dark:bg-rose-400/10 dark:text-rose-300"><X className="h-4 w-4"/>Rechazar</button></>}</div></div></article>)}
    </section>
  </div>;
}
