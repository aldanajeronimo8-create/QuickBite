...
type CancellationRow = { id: string; order_id: string; order_number: string; full_name: string; email: string; reason: string; status: 'pending'|'approved'|'rejected'; refund_amount: number; refund_method: string | null; review_note: string | null; created_at: string; order_item_id: string | null; product_name: string | null; requested_quantity: number | null };
...
export function AdminHistory() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true); setAuditError(null); setCancellationError(null);
    ...
    if (auditResult.status === 'fulfilled') {
      ...
      if (error) setAuditError(error.message); else setAudits((data ?? []) as AuditRow[]);
    } else setAuditError(auditResult.reason instanceof Error ? auditResult.reason.message : 'No se pudo cargar la auditoría remota.');
    if (cancellationResult.status === 'fulfilled') {
      const { data, error } = cancellationResult.value;
      if (error) setCancellationError(error.message); else setCancellations((data ?? []) as CancellationRow[]);
    } else setCancellationError(cancellationResult.reason instanceof Error ? cancellationResult.reason.message : 'No se pudieron cargar las cancelaciones.');
    setLoading(false);
  ...
  return <div className="space-y-7">
    <header className="rounded-[2rem] border border-blue-100 bg-gradient-to-br from-white to-blue-50 p-6 shadow-xl sm:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-blue-700">Trazabilidad</p><h1 className="mt-2 flex items-center gap-3 text-3xl font-black text-slate-950"><History className="h-7 w-7 text-blue-700"/>Auditoría y cancelaciones</h1><p className="mt-2 text-sm text-slate-600">Registro remoto de operaciones y solicitudes de cancelación de pedidos o productos.</p></div><button type="button" onClick={() => void load()} disabled={loading || authLoading || !currentUser} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 shadow-sm"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`}/>Actualizar</button></div></header>
