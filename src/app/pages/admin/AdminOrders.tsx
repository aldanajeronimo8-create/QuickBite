...
import { Check, MessageSquare, ShoppingBag, X, XCircle } from 'lucide-react';
...
const statusLabels: Record<Order['status'], { label: string; className: string }> = {
  ...
  rejected: { label: 'Rechazado', className: 'bg-red-600 text-white' },
  cancelled: { label: 'Cancelado', className: 'bg-slate-500 text-white' },
};
...
function getOrderErrorMessage(error: unknown) {
  ...
  if (message.includes('admin_hidden') || message.includes('column')) {
    return 'Falta aplicar la migración de visibilidad de pedidos en Supabase. Ejecuta la migración y vuelve a intentarlo.';
  }
  ...
  if (/delivered_order_immutable/i.test(message)) return 'El pedido ya fue entregado y su estado no se puede modificar.';
  if (/rejected_order_immutable/i.test(message)) return 'El pedido ya fue rechazado y no se puede modificar.';
  if (/invalid_order_transition/i.test(message)) return 'La transición de estado no es válida.';
  return message;
}
...
function getPaymentBadge(status: Order['payment_status']) {
  const config = paymentLabels[status] ?? paymentLabels.pending;
  return <Badge className={config.className}>Pago: {config.label}</Badge>;
}
...
function getOrderComment(order: Order) {
  const comment = order.notes ?? order.student_comment ?? null;
...
export function AdminOrders() {
  ...
  const filteredOrders = useMemo(() => {
    let next = orders
      .filter((order) => order.payment_status === 'confirmed' || order.payment_status === 'rejected')
      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    if (filterStatus !== 'all') {
      next = next.filter((order) =>
        filterStatus === 'rejected'
          ? order.status === 'rejected' || order.payment_status === 'rejected'
  ...
  const pendingForOrder = (orderId: string) =>
    cancellationRequests.filter((request) => request.order_id === orderId && request.status === 'pending');
  ...
  const handleStatusChange = async (id: string, status: Order['status']) => {
    ...
    try {
      const { updateOrder } = useDataStore.getState();
      await updateOrder(id, { status });
      toast.success(status === 'rejected' ? 'Pedido rechazado y stock restaurado' : 'Estado del pedido actualizado');
  ...
  const reviewCancellation = async (requestId: string, approve: boolean) => {
    ...
    try {
      const { error } = await requireSupabaseClient().rpc('review_order_cancellation', {
        ...
        p_approve: approve,
        p_note: approve
          ? 'Revisado desde Gestión de pedidos.'
          : 'Solicitud rechazada tras revisión desde Gestión de pedidos.',
      });
  ...
  return (
    <div>
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="mb-2 text-4xl font-bold text-green-800">Gestión de pedidos</h1>
          <p className="text-lg text-gray-600">
            Aquí aparecen todos los pedidos con pago confirmado o rechazado. Los pagos pendientes se revisan en Pagos;
            los pedidos con pago confirmado pasan a esta cola operativa.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() => window.location.assign('/admin/reset')}
          >
            Reiniciar período de prueba
          </Button>
        </div>
      </div>
      ...
      <Card className="mb-6 border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="ml-auto text-sm text-gray-600">
            Total: <span className="font-bold text-green-800">{filteredOrders.length}</span> pedido(s)
          </div>
        </div>
      </Card>
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => {
            const rejected = order.status === 'rejected' || order.payment_status === 'rejected';
            const delivered = order.status === 'delivered';
            const terminal = rejected || delivered;
            const pendingCancellationCount = pendingForOrder(order.id).length;
            const comment = getOrderComment(order);
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="mb-2 flex flex-wrap items-center gap-3">
                      ...
                      {pendingCancellationCount > 0 && (
                        <Badge className="bg-red-100 text-red-800">
                          {pendingCancellationCount} cancelación{pendingCancellationCount > 1 ? 'es' : ''} pendiente
                  ...
                  <div className="text-right">
                    ...
                    <p className="mb-3 text-3xl font-bold text-green-800">
                  ...
                  <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 whitespace-pre-wrap break-words text-sm font-medium text-emerald-950">
                          {comment}
                        </p>
                      </div>
                    </div>
                ...
                {!terminal && order.payment_status === 'confirmed' && (
                  <div className="mt-4 border-t border-gray-200 pt-4">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {(['preparing', 'ready', 'delivered'] as const)
                ...
                {pendingCancellationCount > 0 && (
                  <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-red-200 bg-red-50 p-3">
                    <div>
                      <p className="text-sm font-black text-red-900">Hay una solicitud de cancelación pendiente.</p>
                      <p className="text-xs text-red-700">Ábrela desde “Ver detalles” para aprobarla o rechazarla.</p>
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                ...
                {terminal && (
                  <div
                    className={`mt-4 rounded-2xl p-3 text-sm font-semibold ${
                      rejected ? 'bg-red-50 text-red-800' : 'bg-green-50 text-green-800'
                    }`}
                  >
                    {rejected
                      ? 'Pedido rechazado. No se puede modificar.'
                      : 'Pedido entregado. Estado final, sin acciones adicionales.'}
                  </div>
                )}
              </Card>
            );
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          {selectedOrder && (
            <div className="space-y-6">
              ...
              {getOrderComment(selectedOrder) && (
                <section className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-black text-emerald-900">Comentario del estudiante</p>
                      <p className="mt-1 whitespace-pre-wrap break-words text-sm leading-6 text-emerald-950">
                        {getOrderComment(selectedOrder)}
                      </p>
                    </div>
                  </div>
              ...
              <div>
                <h3 className="mb-3 text-lg font-bold text-green-800">Artículos del pedido</h3>
                {selectedOrder.order_items?.length ? (
                  <div className="space-y-2">
                    {selectedOrder.order_items.map((item) => {
                      const request = cancellationRequests.find((row) => row.order_item_id === item.id);
                      return (
                        <div
                          key={item.id}
                          ...
                          <div>
                            ...
                            {request?.status === 'pending' && (
                              <p className="mt-1 text-xs font-bold text-red-700">
                                Solicitud de cancelación pendiente · {request.requested_quantity ?? 1} unidad(es)
                              </p>
                            )}
                          </div>
                  ...
                  <p className="rounded-xl bg-gray-50 p-4 text-sm text-gray-500">
                    Este pedido ya no tiene artículos activos.
                  </p>
                )}
              </div>

              {pendingForOrder(selectedOrder.id).length > 0 && (
                <div className="rounded-2xl border border-red-200 bg-red-50 p-4">
                  <div className="mb-3">
                    <p className="font-black text-red-900">Solicitudes de cancelación</p>
                    <p className="text-xs text-red-700">Puedes decidir aquí sin salir de Gestión de pedidos.</p>
                  </div>
                  <div className="space-y-3">
                    {pendingForOrder(selectedOrder.id).map((request) => (
                      <div key={request.id} className="rounded-xl bg-white p-3 shadow-sm">
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          <div className="min-w-0">
                            <p className="font-black text-slate-900">
                              {request.product_name ? `Producto: ${request.product_name}` : 'Pedido completo'}
                            </p>
                            <p className="mt-1 text-sm text-slate-700">
                              Cantidad solicitada: {request.requested_quantity ?? 1}
                            </p>
                            <p className="mt-1 text-sm text-slate-700">Motivo: {request.reason}</p>
                            <p className="mt-1 text-xs text-slate-500">
                              Solicitud:{' '}
                              {format(new Date(request.created_at), "d 'de' MMMM, yyyy - HH:mm", { locale: es })}
                            </p>
                            <p className="mt-1 text-sm font-bold text-slate-700">
                              Reembolso previsto: ${money(Number(request.refund_amount))}
                            </p>
                          </div>
                          <div className="flex shrink-0 gap-2">
                            <Button
                              size="sm"
                              disabled={reviewingCancellationId === request.id}
                              onClick={() => void reviewCancellation(request.id, true)}
                            ...
                            <Button
                              ...
                              variant="outline"
                              disabled={reviewingCancellationId === request.id}
                              onClick={() => void reviewCancellation(request.id, false)}
                              className="border-red-300 text-red-700 hover:bg-red-50"
                            >
