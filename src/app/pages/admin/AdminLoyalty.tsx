    ...
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mb-8 border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2 md:col-span-2"><Label htmlFor="reward-description">Descripcion</Label><Textarea id="reward-description" value={form.description} maxLength={280} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Ejemplo: Jugo natural de la cafeteria." /></div>
        </div>
        <div className="mt-5 flex justify-end"><Button type="button" onClick={handleSave} disabled={saving} className="bg-blue-700 text-white hover:bg-blue-800">{editingRewardId ? <Save className="h-4 w-4" /> : <Plus className="h-4 w-4" />}{saving ? 'Guardando...' : editingRewardId ? 'Guardar cambios' : 'Crear recompensa'}</Button></div>
      </section>

      <section><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-bold text-blue-900">Recompensas configuradas</h2><span className="text-sm text-slate-500">{rewards.length} total</span></div>{rewards.length === 0 ? <div className="border border-dashed border-slate-200 bg-white px-6 py-12 text-center text-slate-500">Crea la primera recompensa para habilitar canjes.</div> : <div className="grid gap-3 md:grid-cols-2">{rewards.map((reward) => <article key={reward.id} className="flex gap-4 border border-slate-200 bg-white p-4 shadow-sm">{reward.product?.image_url ? <img src={reward.product.image_url} alt={reward.title} className="h-16 w-16 object-cover" /> : <div className="grid h-16 w-16 place-items-center bg-amber-50 text-amber-600"><Gift className="h-6 w-6" /></div>}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><h3 className="truncate font-bold text-slate-900">{reward.title}</h3><p className="mt-1 text-sm text-slate-600">{reward.points_required} puntos - {reward.product?.name}</p></div><span className={reward.active ? 'bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700' : 'bg-slate-100 px-2 py-1 text-xs font-bold text-slate-500'}>{reward.active ? 'Activa' : 'Inactiva'}</span></div><p className="mt-2 line-clamp-2 text-sm text-slate-500">{reward.description || 'Sin descripcion.'}</p><div className="mt-3 flex justify-end gap-2"><Button type="button" size="icon" variant="outline" onClick={() => { setEditingRewardId(reward.id); setForm(toForm(reward)); }} title="Editar recompensa" aria-label="Editar recompensa"><Pencil className="h-4 w-4" /></Button><Button type="button" size="icon" variant="outline" onClick={() => void handleActiveChange(reward)} title={reward.active ? 'Desactivar recompensa' : 'Activar recompensa'} aria-label={reward.active ? 'Desactivar recompensa' : 'Activar recompensa'}><Power className="h-4 w-4" /></Button></div></div></article>)}</div>}</section>

      <section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-bold text-blue-900">Canjes para entregar</h2><span className="text-sm text-slate-500">{redemptions.filter((redemption) => redemption.status === 'reserved').length} pendientes</span></div><div className="overflow-x-auto border border-slate-200 bg-white shadow-sm">{redemptions.length === 0 ? <p className="px-6 py-10 text-center text-slate-500">Aun no hay canjes registrados.</p> : <table className="w-full min-w-[640px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Estudiante</th><th className="px-4 py-3">Recompensa</th><th className="px-4 py-3">Codigo</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{redemptions.map((redemption) => <tr key={redemption.id}><td className="px-4 py-3"><p className="font-medium text-slate-900">{redemption.user?.full_name ?? 'Estudiante'}</p><p className="text-xs text-slate-500">{redemption.user?.email}</p></td><td className="px-4 py-3">{redemption.reward?.title ?? 'Recompensa'}</td><td className="px-4 py-3 font-mono font-bold text-blue-800">{redemption.redemption_code}</td><td className="px-4 py-3"><span className={redemption.status === 'reserved' ? 'bg-amber-50 px-2 py-1 text-xs font-bold text-amber-700' : 'bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600'}>{redemption.status === 'reserved' ? 'Pendiente' : redemption.status === 'fulfilled' ? 'Entregado' : 'Cancelado'}</span></td><td className="px-4 py-3 text-right">{redemption.status === 'reserved' && <Button type="button" size="sm" onClick={() => void handleFulfill(redemption)} disabled={fulfillingRedemptionId === redemption.id} className="bg-emerald-600 text-white hover:bg-emerald-700"><CheckCheck className="h-4 w-4" />{fulfillingRedemptionId === redemption.id ? 'Entregando...' : 'Entregar'}</Button>}</td></tr>)}</tbody></table>}</div></section>
    </div>
  );
}
