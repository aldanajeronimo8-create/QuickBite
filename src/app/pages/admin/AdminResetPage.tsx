...
export function AdminResetPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6">
    <div><p className="text-xs font-black uppercase tracking-[0.2em] text-red-700">Zona crítica</p><h1 className="mt-1 text-4xl font-black text-slate-900">Reiniciar aplicación</h1><p className="mt-2 text-base text-slate-600">Restablece todo el estado transaccional y operativo de QuickBite para comenzar un período nuevo.</p></div>
    <Card className="border border-red-200 bg-white p-6 shadow-sm">
      <div className="flex items-start gap-4"><div className="rounded-2xl bg-red-100 p-3 text-red-700"><AlertTriangle className="h-6 w-6" /></div><div><h2 className="text-xl font-black text-slate-900">Reinicio completo</h2><p className="mt-1 text-sm text-slate-600">Actualmente hay <strong>{activeOrders.length}</strong> pedido(s) visibles en el flujo operativo.</p></div></div>
      <div className="mt-6 rounded-2xl bg-red-50 p-4 text-sm leading-6 text-slate-700"><p className="font-bold text-red-900">Este proceso elimina el período operativo actual de la aplicación.</p><p className="mt-2">Reinicia pedidos y ventas, devuelve al stock lo consumido por pedidos y canjes, borra puntos y canjes del período, vacía transacciones y saldos de billetera, limpia notificaciones y reinicia los datos de informes/analítica operativa.</p><p className="mt-2">Se conservan las cuentas de usuarios, el catálogo de productos y categorías, las recompensas/configuración y los registros de auditoría y salud del sistema.</p></div>
      <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700"><p>La exportación a Excel ejecuta automáticamente este mismo reinicio después de generar correctamente el archivo.</p><p className="mt-2">El archivo Excel descargado es el respaldo del período que acabas de cerrar.</p></div>
