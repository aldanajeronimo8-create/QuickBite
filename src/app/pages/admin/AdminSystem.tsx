...
export function AdminSystem() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { error: healthError } = await client.rpc('run_admin_health_check');
      if (healthError) errors.push(`Health check: ${healthError.message}`);
    } catch (error) {
      errors.push(`Health check: ${error instanceof Error ? error.message : 'fallo desconocido'}`);
    }

    try {
      const { data, error: snapshotError } = await client.rpc('get_system_operational_snapshot');
      if (snapshotError) {
        errors.push(`Snapshot operativo: ${snapshotError.message}`);
      } else {
        const snapshot = (data ?? {}) as SystemSnapshot;
      ...
      errors.push(`Snapshot operativo: ${error instanceof Error ? error.message : 'fallo desconocido'}`);
    }
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">Sistema</p>
        <h1 className="text-3xl font-black text-slate-900">Salud, auditoría y automatizaciones</h1>
        <p className="mt-1 text-sm text-slate-600">Centro de control para comprobar que QuickBite funciona, conserva los datos y registra las operaciones.</p>
      </div>
      <button onClick={() => void load()} disabled={loading || authLoading || !currentUser} className="inline-flex items-center gap-2 rounded-2xl bg-[#1747B8] px-4 py-2 text-sm font-bold text-white shadow-lg disabled:opacity-60">
        <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Actualizar
      </button>
    </div>
    {error && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900">Algunas comprobaciones no estuvieron disponibles: {error}</div>}
