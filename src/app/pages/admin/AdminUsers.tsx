...
export function AdminUsers() {
  ...
  const loadConsents = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('user_id,student_name,guardian_name,guardian_relationship,guardian_email,student_acknowledged,guardian_authorized,privacy_policy_version,consent_at')
  ...
  useEffect(() => {
    let active = true;
    void listProtectedAdminEmails().then((emails) => {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div><h1 className="flex items-center gap-3 text-3xl font-bold text-gray-900"><UserCog className="h-8 w-8 text-blue-600" />Usuarios</h1><p className="mt-1 text-gray-500">Gestiona administradores, estudiantes y la autorización de datos registrada en Supabase.</p></div>
        <Button onClick={beginCreate} className="bg-blue-700 text-white hover:bg-blue-800"><Plus className="mr-2 h-4 w-4" />Crear usuario</Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="w-full min-w-[1100px] text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {filtered.length === 0 && <tr><td colSpan={9} className="px-4 py-10 text-center text-gray-500">No hay usuarios para mostrar.</td></tr>}
          </tbody>
        </table>
      </div>

      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><Card className="w-full max-w-lg border-0 bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-bold text-gray-900">{credentialOnly ? 'Editar credenciales protegidas' : form.id ? 'Editar usuario' : 'Crear usuario'}</h2>{credentialOnly && <p className="mt-1 text-sm text-gray-500">Solo otro administrador puede cambiar las credenciales de una cuenta protegida.</p>}</div><button onClick={() => setOpen(false)} className="rounded-full p-2 text-gray-400 hover:bg-gray-100"><X className="h-5 w-5" /></button></div>
        <form onSubmit={saveUser} className="space-y-4">
          {!credentialOnly && <div><Label>Nombre completo</Label><Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} /></div>}
          ...
          {!credentialOnly && <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><div><Label>Rol</Label><select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as Profile['role'] })} className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="student">Estudiante</option><option value="admin">Admin</option><option value="both">Admin y estudiante</option></select></div><div><Label>TI</Label><Input value={form.ti} onChange={(e) => setForm({ ...form, ti: e.target.value })} disabled={form.role === 'admin'} /></div></div>}
          <div className="flex justify-end gap-2 pt-2"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button type="submit" disabled={saving} className="bg-blue-700 text-white hover:bg-blue-800">{saving ? 'Guardando...' : 'Guardar'}</Button></div>
        </form>
      </Card></div>}
    </div>
  );
