...
export function AdminOperations() {
 ...
 const load=useCallback(async()=>{
  ...
  const [slotResult,rankingResult,inventoryResult]=results;
 ...
 const saveSlot=async()=>{if(!currentUser)return; if(!form.name.trim()){toast.error('El nombre de la ventana es obligatorio.');return;} if(form.starts===form.ends){toast.error('La hora de inicio y fin deben ser diferentes.');return;} setSaving(true);try{const c=requireSupabaseClient();const {error}=await c.rpc('admin_upsert_pickup_slot',{p_id:null,p_name:form.name.trim(),p_starts_at:form.starts,p_ends_at:form.ends,p_enabled:true,p_max_orders:form.max?Number(form.max):null});if(error)throw error;toast.success('Ventana de pedidos creada.');await load()}catch(e){toast.error(e instanceof Error?e.message:'No se pudo guardar la ventana.')}finally{setSaving(false)}};
 const toggleSlot=async(s:Slot)=>{if(!currentUser)return;try{const {error}=await requireSupabaseClient().rpc('admin_upsert_pickup_slot',{p_id:s.slot_id,p_name:s.slot_name,p_starts_at:s.starts_at,p_ends_at:s.ends_at,p_enabled:!s.enabled,p_max_orders:s.max_orders});if(error)throw error;toast.success(s.enabled?'Ventana desactivada.':'Ventana activada.');await load()}catch(e){toast.error(e instanceof Error?e.message:'No se pudo actualizar la ventana.')}};
 ...
 <section className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><Clock3 className="h-5 w-5 text-blue-700"/><div><h2 className="text-xl font-black">Ventanas de pedidos</h2><p className="text-sm text-slate-500">Define cuándo se aceptan pedidos y el máximo de pedidos por ventana.</p></div></div><div className="mt-4 grid gap-2 md:grid-cols-5"><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Nombre" className="rounded-xl border p-3 text-sm"/><input type="time" value={form.starts} onChange={e=>setForm({...form,starts:e.target.value})} className="rounded-xl border p-3 text-sm"/><input type="time" value={form.ends} onChange={e=>setForm({...form,ends:e.target.value})} className="rounded-xl border p-3 text-sm"/><input type="number" min="1" value={form.max} onChange={e=>setForm({...form,max:e.target.value})} placeholder="Máximo" className="rounded-xl border p-3 text-sm"/><button disabled={saving||authLoading||!currentUser} onClick={()=>void saveSlot()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 py-3 text-sm font-black text-white disabled:opacity-50"><Plus className="h-4 w-4"/>Crear</button></div><div className="mt-4 space-y-2">{slots.map(s=><div key={s.slot_id} className="flex flex-col gap-2 rounded-2xl bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black">{s.slot_name} · {s.starts_at.slice(0,5)}–{s.ends_at.slice(0,5)}</p><p className="text-xs text-slate-500">{s.orders_count} pedidos hoy · límite {s.max_orders??'sin límite'} · {s.accepting_orders?'Aceptando ahora':'Cerrada ahora'}</p></div><button onClick={()=>void toggleSlot(s)} disabled={authLoading||!currentUser} className={`rounded-xl px-3 py-2 text-xs font-black disabled:opacity-50 ${s.enabled?'bg-emerald-100 text-emerald-800':'bg-slate-200 text-slate-600'}`}>{s.enabled?'Activa':'Desactivada'}</button></div>)}{!loading&&slots.length===0&&<p className="rounded-2xl border border-dashed p-5 text-sm text-slate-500">No hay ventanas configuradas.</p>}</div></section>
