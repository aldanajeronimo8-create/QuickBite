...
const dateTime = (value: string) => new Date(value).toLocaleString('es-CO', { dateStyle: 'short', timeStyle: 'short' });
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-4 py-3 font-black shadow-sm disabled:cursor-not-allowed disabled:opacity-60"
            aria-label="Actualizar recargas"
          >
        ...
        <div className="rounded-3xl bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black">Pendientes</p>
              <p className="text-sm text-slate-500">{pending.length} solicitudes por revisar</p>
            </div>
          </div>
        </div>
        ...
        {error && !loading && (
          <div className="rounded-3xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-700">
            <p className="font-black">No se pudieron cargar las solicitudes.</p>
            <p className="mt-1">{error}</p>
            <button onClick={() => void load(true)} className="mt-3 rounded-full bg-white px-4 py-2 font-black shadow-sm">Reintentar</button>
          </div>
        )}

        {loading ? (
          <div className="rounded-3xl bg-white p-8 text-center text-sm font-bold text-slate-500">Cargando solicitudes…</div>
        ) : requests.length === 0 ? (
          <div className="rounded-3xl bg-white p-8 text-center">No hay solicitudes de recarga.</div>
        ) : (
                ...
                <div className="grid gap-5 lg:grid-cols-[1fr_auto]">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-lg font-black">{r.full_name || 'Usuario sin nombre'}</p>
                        <p className="text-sm text-slate-500">{r.email || r.user_id}</p>
                      </div>
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black uppercase text-slate-600">{r.status}</span>
                    </div>
                    ...
                    <div className="mt-4 flex gap-3 rounded-2xl bg-slate-50 p-4">
                      ...
                      <div className="min-w-0"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Comentario</p><p className="mt-1 whitespace-pre-wrap break-words text-sm text-slate-700">{r.comment || 'Sin comentario'}</p></div>
                    </div>
                    {r.status === 'pending' && (
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button disabled={busy === r.id} onClick={() => void approve(r.id)} className="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-4 py-2 text-sm font-black text-white disabled:opacity-50"><Check className="h-4 w-4" />Aprobar</button>
                        <button disabled={busy === r.id} onClick={() => void reject(r.id)} className="inline-flex items-center gap-2 rounded-full bg-rose-600 px-4 py-2 text-sm font-black text-white disabled:opacity-50"><X className="h-4 w-4" />Rechazar</button>
                      </div>
                    )}
