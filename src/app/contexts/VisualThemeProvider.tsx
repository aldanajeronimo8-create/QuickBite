...
function applyDocumentTheme(settings: VisualSettings) {
  ...
  root.style.setProperty('--qb-header-style', settings.header_style); root.style.setProperty('--qb-navigation-style', settings.navigation_style); root.style.setProperty('--qb-card-style', settings.card_style); root.style.setProperty('--qb-input-style', settings.input_style);
  root.classList.toggle('dark', settings.theme_mode === 'dark'); root.dataset.qbTheme = settings.theme_mode; document.title = settings.app_name;
  let link = document.querySelector<HTMLLinkElement>('link[data-quickbite-favicon]'); if (!link) { link = document.createElement('link'); link.rel = 'icon'; link.dataset.quickbiteFavicon = 'true'; document.head.appendChild(link); } link.href = settings.favicon_url || '/favicon.ico';
}
