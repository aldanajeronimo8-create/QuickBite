...
const FONT_OPTIONS = ['Nunito', 'Inter', 'Poppins', 'Roboto', 'Montserrat', 'Open Sans', 'Lato', 'Raleway', 'Merriweather', 'Playfair Display', 'Source Sans 3', 'Ubuntu', 'DM Sans', 'Manrope', 'Work Sans', 'system-ui', 'ui-sans-serif', 'Arial', 'Georgia', 'Verdana', 'Trebuchet MS', 'serif'];
...
const UNIVERSAL_PRESETS: Array<{ id: string; label: string; description: string; style: VisualElementStyle }> = [
  ...
  { id: 'outline-green', label: 'Outline verde', description: 'Solo borde', style: { backgroundColor: '#FFFFFF', color: '#16A36A', borderColor: '#16A36A', borderStyle: 'solid', borderWidth: '2px' } },
  { id: 'outline-blue', label: 'Outline azul', description: 'Solo borde', style: { backgroundColor: '#FFFFFF', color: '#2563EB', borderColor: '#2563EB', borderStyle: 'solid', borderWidth: '2px' } },
  { id: 'ghost', label: 'Ghost', description: 'Visual ligero', style: { backgroundColor: '#F8FAFC', color: '#334155', borderColor: '#E2E8F0', borderStyle: 'solid', borderWidth: '1px' } },
  ...
  { id: 'dashed', label: 'Discontinuo', description: 'Borde dashed', style: { backgroundColor: '#FFFFFF', color: '#334155', borderColor: '#94A3B8', borderStyle: 'dashed', borderWidth: '2px' } },
  { id: 'double', label: 'Doble', description: 'Borde doble', style: { backgroundColor: '#FFFFFF', color: '#0F172A', borderColor: '#0F172A', borderStyle: 'double', borderWidth: '3px' } },
...
const BUTTON_PRESETS = [...UNIVERSAL_PRESETS, ...[
...
const SHAPES = [
  ['square', 'Cuadrado', '0px'], ['soft', 'Suave', '6px'], ['rounded', 'Redondeado', '12px'], ['curved', 'Curvo', '16px'], ['large-rounded', 'Grande', '20px'],
...
function isDynamicId(value: string) { return value.startsWith('radix-') || value.startsWith('headlessui-') || value.startsWith(':r') || value.length > 20 || value.split('').every(char => char >= '0' && char <= '9'); }
function attrSelector(name: string, value: string) { const safe = value.replace(/\\/g, '\\\\').replace(/"/g, '\\"'); return `[${name}="${safe}"]`; }
function selectorMatchesOnlyElement(selector: string, element: Element) { try { const matches = document.querySelectorAll(selector); return matches.length === 1 && matches[0] === element; } catch { return false; } }
function getNthOfType(element: Element) { if (!element.parentElement) return ''; const siblings = Array.from(element.parentElement.children).filter(child => child.tagName === element.tagName); return siblings.length > 1 ? `:nth-of-type(${siblings.indexOf(element) + 1})` : ''; }
function buildSelector(element: Element) {
  const parts: string[] = [];
  let current: Element | null = element;
  while (current && current !== document.documentElement && current !== document.body) {
    let part = current.tagName.toLowerCase();
    const testId = current.getAttribute('data-testid'); const slot = current.getAttribute('data-slot'); const name = current.getAttribute('name'); const aria = current.getAttribute('aria-label'); const id = (current as HTMLElement).id;
    ...
    else { const classCandidate = classes.slice(0, 4).map(cssEscape).join('.'); if (classCandidate && selectorMatchesOnlyElement(`${part}.${classCandidate}`, current)) part += `.${classCandidate}`; else part += getNthOfType(current); }
    parts.unshift(part);
    const candidate = parts.join(' > '); if (selectorMatchesOnlyElement(candidate, element)) return candidate;
...
function FieldEditor({ field, value, onCommit }: FieldEditorProps) {
  const stringValue = String(value ?? ''); const colorValue = typeof value === 'string' && /^#[0-9a-f]{6}$/i.test(value) ? value : '#ffffff'; const update = (next: string) => onCommit({ [field.key]: next } as VisualElementStyle);
  return <label className="block"><span className="mb-1 block text-xs font-black qb-text-secondary">{field.label}</span>{field.hint && <span className="mb-1 block text-[10px] qb-text-muted">{field.hint}</span>}{field.type === 'color' && <div className="flex gap-2"><input aria-label={field.label} type="color" value={colorValue} onChange={e => update(e.target.value)} className="size-10 cursor-pointer rounded-lg border qb-border p-1" /><input aria-label={`${field.label} hexadecimal`} value={stringValue} onChange={e => update(e.target.value)} className="min-w-0 flex-1 rounded-xl border qb-border qb-surface-muted px-3 py-2 text-sm qb-text" /></div>}{field.type === 'select' && <select value={stringValue} onChange={e => update(e.target.value)} className="w-full rounded-xl border qb-border qb-surface-muted px-3 py-2 text-sm qb-text">{field.options?.map(option => <option key={option} value={option}>{option}</option>)}</select>}{field.type === 'textarea' && <textarea value={stringValue} onChange={e => update(e.target.value)} className="min-h-20 w-full resize-y rounded-xl border qb-border qb-surface-muted px-3 py-2 text-sm qb-text" maxLength={500} />}{field.type === 'text' && <input value={stringValue} onChange={e => update(e.target.value)} className="w-full rounded-xl border qb-border qb-surface-muted px-3 py-2 text-sm qb-text" />}</label>;
}
...
function InspectorSection({ id, title, openSection, setOpenSection, children }: SectionProps) { const open = openSection === id; return <div className="overflow-hidden rounded-2xl border qb-border"><button type="button" onClick={() => setOpenSection(open ? null : id)} className="flex w-full items-center justify-between qb-surface-muted px-4 py-3 text-left"><span className="text-[11px] font-black uppercase tracking-[.12em] qb-text">{title}</span><ChevronDown className={`size-4 qb-text-muted transition ${open ? 'rotate-180' : ''}`} /></button>{open && <div className="p-4">{children}</div>}</div>; }
function PresetGrid({ presets, current, onApply, compact = false }: { presets: ReadonlyArray<{ id: string; label: string; description: string; style: VisualElementStyle }>; current: VisualElementStyle; onApply: (style: VisualElementStyle) => void; compact?: boolean }) { return <div className={`grid ${compact ? 'grid-cols-3' : 'grid-cols-2'} gap-2`}>{presets.map(preset => { const active = preset.style.backgroundColor === current.backgroundColor && preset.style.color === current.color && preset.style.borderStyle === current.borderStyle && (!preset.style.borderRadius || preset.style.borderRadius === current.borderRadius); return <button key={preset.id} type="button" onClick={() => onApply(preset.style)} className={`group rounded-xl border p-2 text-left transition ${active ? 'border-emerald-500 bg-emerald-50/80 dark:border-emerald-300/40 dark:bg-emerald-500/10' : 'qb-border qb-surface hover:border-slate-400'}`}><span className="flex h-9 items-center justify-center rounded-lg border text-[10px] font-black" style={{ backgroundColor: preset.style.backgroundColor, backgroundImage: preset.style.backgroundImage, color: preset.style.color, borderColor: preset.style.borderColor, borderStyle: preset.style.borderStyle, borderWidth: preset.style.borderWidth, borderRadius: preset.style.borderRadius }}>{preset.label}</span><span className="mt-1 block text-[9px] qb-text-muted">{preset.description}</span></button>; })}</div>; }
