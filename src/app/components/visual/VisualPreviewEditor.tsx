...
export function VisualPreviewEditor({ scope }: Props) {
  ...
  // Prefer the draft overrides for this interface. The global map is retained
  // as a fallback for compatibility with older saved configurations.
  const scopedOverrides = settings.interface_overrides?.[scope]?.element_overrides ?? {};
  const existingOverrides = Object.keys(scopedOverrides).length ? scopedOverrides : (settings.element_overrides ?? {});
  ...
  useEffect(() => {
    const handleSelection = (event: MouseEvent) => {
      ...
      const editable = interactive ?? target.closest('*') as HTMLElement | null;
  ...
  // This effect applies only values that are explicitly present in the draft.
  // Inspection itself never converts computed styles into inline overrides.
  useEffect(() => {
    if (!selected) return;
  ...
  const commit = (patch: VisualElementStyle) => {
    ...
    setDraftStyle(next);
    const fullOverrides = { ...existingOverrides, [selected.selector]: next };
    const fullSettings = buildDraftSettings(fullOverrides);
    const payload = { type: 'quickbite-visual-element-edit', scope, selector: selected.selector, styles: next, settings: fullSettings };
    window.postMessage(payload, window.location.origin);
    window.opener?.postMessage(payload, window.location.origin);
    try { localStorage.setItem(PREVIEW_STORAGE_KEY, JSON.stringify(fullSettings)); } catch { /* ignore */ }
    setMessage('Cambio aplicado al borrador. Puedes seguir editando y guardar todo al final.');
  };
  ...
  const reset = () => {
    ...
    const payload = { type: 'quickbite-visual-element-reset', scope, selector: selected.selector, settings: fullSettings };
    window.postMessage(payload, window.location.origin);
    window.opener?.postMessage(payload, window.location.origin);
    try { localStorage.setItem(PREVIEW_STORAGE_KEY, JSON.stringify(fullSettings)); } catch { /* ignore */ }
    setMessage('Estilo restablecido dentro del borrador.');
  };
