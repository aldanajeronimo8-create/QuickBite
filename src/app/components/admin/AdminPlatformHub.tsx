...
export function AdminPlatformHub() {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [stockRows, alertRows, notificationRows, salesRows, summaryRow, staffRole] = await Promise.all([
        listLowStockProducts(), listOpenAlerts(), listNotifications(user.id), getDailySales(14), getDailySummary(), getStaffRole(user.id),
      ]);
