...
export function AdminPlatformHub() {
  ...
  if (!user) return null;
  const todaySales = summary ? Number(summary.sales_total ?? summary.total_sales ?? 0) : 0;
  const todayOrders = summary ? Number(summary.orders_count ?? summary.total_orders ?? 0) : 0;
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 bg-slate-950/40 p-4" onMouseDown={() => setOpen(false)}><section className="mx-auto mt-8 max-h-[88vh] max-w-5xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Metric icon={BarChart3} label="Ventas 14 días" value={sales.length ? `${sales.length} días` : 'Sin datos'} /><Metric icon={Boxes} label="Stock crítico" value={String(lowStock.length)} /><Metric icon={Bell} label="Alertas abiertas" value={String(alerts.length)} /><Metric icon={ShieldCheck} label="Pedidos hoy" value={String(todayOrders)} /><Metric icon={TrendingUp} label="Demanda 14 días" value={String(demandTotal)} /></div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2"><section className="rounded-2xl border p-4"><h3 className="mb-3 flex items-center gap-2 font-black"><AlertTriangle className="h-5 w-5" /> Alertas y stock</h3>{alerts.length ? <div className="space-y-2">{alerts.slice(0, 5).map((alert) => <div key={alert.id} className="flex items-start justify-between gap-3 rounded-xl bg-amber-50 p-3"><div><p className="font-bold">{alert.title ?? 'Alerta'}</p><p className="text-sm text-slate-600">{alert.message}</p></div><button className="text-xs font-bold text-blue-700" onClick={() => void dismissAlert(alert.id)}>Resolver</button></div>)}</div> : <p className="text-sm text-slate-500">No hay alertas abiertas.</p>}{lowStock.length > 0 && <div className="mt-3 space-y-2">{lowStock.slice(0, 5).map((product) => <div key={product.id} className="flex justify-between rounded-xl bg-slate-50 p-3 text-sm"><span className="font-bold">{product.name}</span><span className="font-black text-amber-700">Stock {product.stock}</span></div>)}</div>}</section><section className="rounded-2xl border p-4"><h3 className="mb-3 flex items-center gap-2 font-black"><TrendingUp className="h-5 w-5" /> Resumen operativo</h3><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Ventas hoy</p><p className="text-xl font-black">${todaySales.toLocaleString('es-CO')}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Notificaciones</p><p className="text-xl font-black">{notifications.length}</p></div></div><p className="mt-3 text-xs text-slate-500">Demanda registrada: {demand.length} registros · {demandTotal} unidades en los últimos 14 días.</p></section></div>
    </section></div>}
  </>;
}
function Metric({ icon: Icon, label, value }: { icon: typeof Activity; label: string; value: string }) { return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><Icon className="mb-2 h-5 w-5 text-blue-700" /><p className="text-xs font-bold text-slate-500">{label}</p><p className="text-xl font-black">{value}</p></div>; }
