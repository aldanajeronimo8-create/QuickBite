...
export function StudentFeatureHub({ userId }: FeatureHubProps) {
  ...
  async function refresh() {
    ...
    try {
      ...
      setFavorites(nextFavorites); setNotifications(notificationRows ?? []); setHistory((historyRows ?? []) as typeof history); setSlots(slotRows ?? []);
  ...
  async function toggleFavorite(product: Product) {
    ...
    try { await setFavorite(userId, product.id, !exists); setFavorites((current) => exists ? current.filter((item) => item.id !== product.id) : [...current, product]); toast.success(exists ? 'Quitado de favoritos' : 'Añadido a favoritos'); }
    catch (error) { toast.error(error instanceof Error ? error.message : 'No se pudo actualizar favoritos.'); }
  }

  async function prepareReorder(orderId: string) {
    try { const items = await getReorderItems(orderId); if (!items.length) { toast.info('Los productos de ese pedido ya no están disponibles.'); return; } toast.success(`${items.length} producto(s) disponibles. Abre el menú para agregarlos al carrito.`); setOpen(false); }
    catch (error) { toast.error(error instanceof Error ? error.message : 'No se pudo preparar la recompra.'); }
  }

  async function scheduleSelectedOrder() {
    if (!selectedOrder || !selectedSlot) return; const slot = slots.find((item) => item.id === selectedSlot); if (!slot) return;
    try { await scheduleOrder(selectedOrder, selectedSlot, slot.starts_at); toast.success('Pedido programado correctamente.'); await refresh(); }
    catch (error) { toast.error(error instanceof Error ? error.message : 'No se pudo programar el pedido.'); }
  }
