...
import { getReorderItems, listFavorites, listNotifications, listOrderHistory, listPickupSlots, scheduleOrder, setFavorite } from '../../../services/platformFeatures';
...
export function StudentFeatureHub({ userId }: FeatureHubProps) {
  ...
  const [notifications, setNotifications] = useState<Array<{ id: string; title?: string; message?: string; read_at?: string | null }>>([]);
  const [history, setHistory] = useState<Array<{ id: string; order_number: string; created_at: string; total: number; status: string }>>([]);
  const [slots, setSlots] = useState<Array<{ id: string; name?: string; starts_at: string; ends_at?: string }>>([]);
  ...
  async function refresh() {
    ...
    try {
      ...
      setHistory((historyRows ?? []) as typeof history);
      setSlots(slotRows ?? []);
  ...
  async function prepareReorder(orderId: string) {
    try {
      ...
      toast.success(`${items.length} producto(s) disponibles. Abre el menú para agregarlos al carrito.`);
      setOpen(false);
    } catch (error) {
  ...
  async function scheduleSelectedOrder() {
    ...
    if (!slot) return;
    try {
      await scheduleOrder(selectedOrder, selectedSlot, slot.starts_at);
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 left-5 z-30 flex items-center gap-2 rounded-full bg-green-700 px-4 py-3 text-sm font-black text-white shadow-xl ring-4 ring-white/80 transition hover:bg-green-800"
        aria-label="Abrir funciones de QuickBite"
      >
        <Star className="h-4 w-4" /> Mi QuickBite {unread > 0 && <Badge className="bg-white text-green-800">{unread}</Badge>}
      </button>

      {open && (
        <div className="fixed inset-0 z-40 bg-slate-950/40 p-4" onMouseDown={() => setOpen(false)}>
          <section className="mx-auto mt-10 max-h-[85vh] max-w-3xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="mb-5 flex items-center justify-between gap-3">
              <div><p className="text-xs font-black uppercase tracking-[0.2em] text-green-700">Centro del estudiante</p><h2 className="text-2xl font-black text-slate-950">Tus funciones rápidas</h2></div>
              <div className="flex gap-2"><Button variant="outline" size="icon" onClick={() => void refresh()} disabled={loading} aria-label="Actualizar"><RefreshCw className="h-4 w-4" /></Button><Button variant="outline" size="icon" onClick={() => setOpen(false)} aria-label="Cerrar"><X className="h-4 w-4" /></Button></div>
            </div>
