          ...
          {rewards.map((reward) => {
            ...
            return (
              <article key={reward.id} className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                {reward.product?.image_url ? <img src={reward.product.image_url} alt={reward.title} className="h-20 w-20 shrink-0 object-cover" /> : <div className="grid h-20 w-20 shrink-0 place-items-center bg-green-50 text-green-600"><Gift className="h-7 w-7" /></div>}
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div><h3 className="font-black">{reward.title}</h3><p className="mt-1 text-sm text-slate-600">{reward.description || reward.product?.name}</p></div>
                    <span className="shrink-0 bg-green-50 px-2 py-1 text-xs font-black text-green-700">{reward.points_required} pts</span>
                  </div>
                  <Button type="button" onClick={() => onRedeem(reward)} disabled={!canRedeem || redeeming} className="mt-3 w-full bg-green-600 text-white hover:bg-green-700">
        ...
        <section>
          ...
          <div className="divide-y border border-slate-100 bg-white">
            {redemptions.slice(0, 5).map((redemption) => {
              const active = ['reserved', 'pending', 'approved'].includes(redemption.status);
              return (
                <div key={redemption.id} className="flex items-center gap-3 px-4 py-3">
                  {active ? <TicketCheck className="h-5 w-5 text-emerald-500" /> : <LockKeyhole className="h-5 w-5 text-slate-400" />}
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-bold">{redemption.reward?.title ?? 'Recompensa'}</p>
                    <p className="text-xs text-slate-500">Codigo: {redemption.redemption_code ?? 'Generado'}</p>
                  </div>
                  <span className="text-xs font-bold text-slate-500">{active ? 'Para recoger' : redemption.status}</span>
                </div>
              );
            })}
          </div>
