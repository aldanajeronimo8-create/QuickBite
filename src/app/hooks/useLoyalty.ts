...
export function useLoyalty(userId: string | undefined, orders: Order[]) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setError(null);
      const [nextSettings, nextRewards, nextRedemptions] = await Promise.all([getLoyaltySettings(), listLoyaltyRewards(), listUserLoyaltyRedemptions(userId)]);
      setSettings(nextSettings); setRewards(nextRewards); setRedemptions(nextRedemptions);
    } catch (nextError) { setError(getErrorMessage(nextError, 'No se pudo cargar el programa de puntos.')); }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`loyalty-${userId}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'loyalty_redemptions', filter: `user_id=eq.${userId}` }, () => void refresh())
      .subscribe();
  ...
  const earnedPoints = useMemo(() => {
    ...
    const confirmedTotal = orders.filter((order) => order.payment_status === 'confirmed').reduce((sum, order) => sum + Number(order.total), 0);
    return Math.floor(confirmedTotal / unit);
  }, [orders, settings?.points_per_currency_unit]);
  const spentPoints = useMemo(() => redemptions.filter((redemption) => redemption.status === 'reserved' || redemption.status === 'fulfilled').reduce((sum, redemption) => sum + redemption.points_spent, 0), [redemptions]);
  const redeem = useCallback(async (rewardId: string) => { const redemption = await redeemLoyaltyReward(rewardId); await refresh(); return redemption; }, [refresh]);
