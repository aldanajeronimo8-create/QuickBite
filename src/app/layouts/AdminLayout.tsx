...
export function AdminLayout() {
  ...
  return <div className="admin-shell min-h-screen" style={{ background: '#F8FAFC' }}>
    ...
    <main className={`${mainMargin} min-h-screen transition-[margin] duration-200`}><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur-xl lg:px-8"><div className="flex items-center gap-3"><button type="button" onClick={() => setMobileSidebarOpen(true)} className="rounded-xl p-2 text-slate-600 hover:bg-slate-100 lg:hidden" aria-label="Abrir menú"><Menu className="h-5 w-5" /></button><div><p className="text-xs font-black uppercase tracking-[.18em] text-slate-400">Administración</p><h1 className="text-base font-black text-slate-900">{location.pathname === '/admin' ? 'Dashboard' : (sectionPath[sectionForPath(location.pathname)] ?? 'Administración')}</h1></div></div><div className="flex items-center gap-2"><button type="button" onClick={() => setNotificationOpen((open) => !open)} className="relative rounded-xl p-2 text-slate-600 hover:bg-slate-100" aria-label="Notificaciones"><Bell className="h-5 w-5" />{unreadTotal > 0 && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-red-500 px-1 text-[9px] font-black leading-4 text-white">{unreadTotal > 9 ? '9+' : unreadTotal}</span>}</button>{notificationOpen && <div className="absolute right-4 top-14 z-50 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-4 py-3"><p className="text-sm font-black text-slate-900">Notificaciones</p><button type="button" onClick={() => void markAllNotificationsRead()} disabled={preview || notifications.length === 0 || isClearingNotifications} className="inline-flex items-center gap-1 text-xs font-bold text-blue-700 disabled:opacity-40"><CheckCheck className="h-3.5 w-3.5" />Marcar leídas</button></div><div className="max-h-80 overflow-auto">{notifications.length===0?<p className="p-5 text-sm text-slate-500">No hay notificaciones pendientes.</p>:notifications.map((item)=><button key={item.id} type="button" onClick={() => { setNotificationOpen(false); navigate(sectionPath[item.section] ?? '/admin'); }} className="block w-full border-b border-slate-100 p-4 text-left hover:bg-slate-50"><p className="text-sm font-black text-slate-800">{item.title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{item.body}</p></button>)}</div></div>}</div></header><div className="p-4 lg:p-8"><Outlet /></div></main>
