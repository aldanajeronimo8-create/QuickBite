...
import { BarChart3, CreditCard, Gift, GraduationCap, LayoutDashboard, LogOut, Menu, Package, ScanLine, ShoppingBag, Users, UtensilsCrossed, X } from 'lucide-react';
...
function NavItem({ path, label, icon: Icon, active, badge, collapsed = false }: NavItemProps) {
  return <Link to={path} title={collapsed ? label : undefined} className={`admin-nav-link mb-0.5 flex items-center rounded-lg py-2.5 text-sm font-medium transition-all ${collapsed ? 'justify-center px-2' : 'gap-3 px-3'}`} style={active ? { background: 'rgba(255,255,255,0.13)', color: '#fff', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.15)' } : { color: '#BFDBFE' }}><Icon className="h-4 w-4 shrink-0" />{!collapsed && <span className="flex-1">{label}</span>}{!collapsed && badge != null && badge > 0 && <span className="rounded-full px-1.5 py-0.5 text-xs font-bold" style={{ background: navigationAccent, color: primaryColor }}>{badge}</span>}{collapsed && badge != null && badge > 0 && <span className="absolute ml-6 mt-[-18px] h-2.5 w-2.5 rounded-full" style={{ background: navigationAccent }} aria-label={`${badge} pendientes`} />}</Link>;
}
...
export function AdminLayout() {
  ...
  const sidebarWidth = sidebarCollapsed ? 'lg:w-20' : 'lg:w-64'; const mainMargin = sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64';
  return <div className="admin-shell min-h-screen" style={{ background: '#F8FAFC' }}>
    ...
    <aside className={`admin-sidebar fixed left-0 top-0 z-40 flex h-full w-72 flex-col shadow-xl transition-all duration-200 lg:w-64 ${sidebarWidth} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} style={{ background: primaryColor }}>
      <div className={`px-5 pb-5 pt-7 ${sidebarCollapsed ? 'lg:px-3' : ''}`}><div className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'gap-3'}`}><QuickBiteLogo className="h-10 w-10 shrink-0 rounded-xl shadow-md" alt="QuickBite Administración" />{!sidebarCollapsed && <div className="min-w-0"><p className="text-base font-bold leading-tight text-white">QuickBite Admin</p><p className="text-xs" style={{ color: '#93C5FD' }}>Panel de control</p></div>}<button type="button" className="ml-auto rounded-lg p-2 text-blue-100 hover:bg-white/10 lg:hidden" onClick={() => setMobileSidebarOpen(false)} aria-label="Cerrar menú lateral"><X className="h-5 w-5" /></button></div></div>
      ...
      <nav className={`flex-1 overflow-y-auto px-3 ${sidebarCollapsed ? 'lg:px-2' : ''}`}>
        ...
        <NavItem path="/admin/reports" label="Informes" icon={BarChart3} active={isCurrentPath('/admin/reports')} collapsed={sidebarCollapsed} />
      </nav>
      <div className={`px-4 pb-5 pt-3 ${sidebarCollapsed ? 'lg:px-2' : ''}`} style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        {!sidebarCollapsed && <div className="mb-3 flex items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold text-white" style={{ background: 'rgba(255,255,255,0.12)' }}>{user.full_name?.[0]?.toUpperCase() ?? 'A'}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-white">{user.full_name}</p><p className="truncate text-xs" style={{ color: '#93C5FD' }}>{user.role === 'both' ? 'Administrador y estudiante' : 'Administrador'}</p></div></div>}
        {canAccessStudent(user.role) && <Link to="/menu" title={sidebarCollapsed ? 'Ver como estudiante' : undefined} className={`mb-2 flex w-full items-center rounded-md border py-2 text-xs font-semibold transition hover:bg-white/10 ${sidebarCollapsed ? 'justify-center px-2' : 'justify-center px-3'}`} style={{ borderColor: 'rgba(255,255,255,0.2)', color: '#DBEAFE' }}><GraduationCap className={sidebarCollapsed ? 'h-4 w-4' : 'mr-2 h-3.5 w-3.5'} />{!sidebarCollapsed && 'Ver como estudiante'}</Link>}
        <Button onClick={handleSignOut} variant="outline" size="sm" title={sidebarCollapsed ? 'Cerrar sesión' : undefined} className="w-full text-xs" style={{ background: 'transparent', borderColor: 'rgba(255,255,255,0.2)', color: '#BFDBFE' }}><LogOut className={sidebarCollapsed ? 'h-4 w-4' : 'mr-2 h-3.5 w-3.5'} />{!sidebarCollapsed && 'Cerrar sesión'}</Button>
      </div>
    </aside>
    <header className="admin-mobile-header sticky top-0 z-20 flex h-14 items-center border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur lg:hidden"><button type="button" onClick={() => setMobileSidebarOpen(true)} className="rounded-xl p-2 text-blue-900 hover:bg-blue-50" aria-label="Abrir menú lateral"><Menu className="h-6 w-6" /></button><div className="ml-3 flex items-center gap-2"><QuickBiteLogo className="h-8 w-8 rounded-lg" alt="QuickBite" /><span className="font-black text-blue-900">QuickBite Admin</span></div></header>
