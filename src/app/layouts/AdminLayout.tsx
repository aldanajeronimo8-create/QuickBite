...
function NavItem({ path, label, icon: Icon, active, badge, collapsed = false }: NavItemProps) {
  return <Link to={path} title={collapsed ? label : undefined} className={`mb-0.5 flex items-center rounded-lg py-2.5 text-sm font-medium transition-all ${collapsed ? 'justify-center px-2' : 'gap-3 px-3'}`} style={active ? { background: 'rgba(255,255,255,0.13)', color: '#fff', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.15)' } : { color: '#BFDBFE' }}><Icon className="h-4 w-4 shrink-0" />{!collapsed && <span className="flex-1">{label}</span>}{!collapsed && badge != null && badge > 0 && <span className="rounded-full px-1.5 py-0.5 text-xs font-bold" style={{ background: navigationAccent, color: primaryColor }}>{badge}</span>}{collapsed && badge != null && badge > 0 && <span className="absolute ml-6 mt-[-18px] h-2.5 w-2.5 rounded-full" style={{ background: navigationAccent }} aria-label={`${badge} pendientes`} />}</Link>;
}
...
export function AdminLayout() {
  ...
  return <div className="min-h-screen" style={{ background: '#F8FAFC' }}>
    ...
    <aside className={`fixed left-0 top-0 z-40 flex h-full w-72 flex-col shadow-xl transition-all duration-200 lg:w-64 ${sidebarWidth} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} style={{ background: primaryColor }}>
