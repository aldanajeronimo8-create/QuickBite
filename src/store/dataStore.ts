...
const REALTIME_TABLES = [
  'profiles', 'categories', 'products', 'orders', 'order_items', 'notifications',
  'loyalty_settings', 'loyalty_rewards', 'loyalty_redemptions', 'favorites', 'pickup_slots',
  'system_alerts', 'product_stock_settings', 'daily_summaries', 'staff_roles',
  'demand_observations', 'automation_jobs',
] as const;
...
interface DataState {
  categories: Category[]; products: Product[]; orders: Order[]; users: Profile[]; history: HistoryEntry[]; loading: boolean;
  loadData: (options?: { silent?: boolean }) => Promise<void>;
  addProduct: (product: repo.NewProduct) => Promise<void>; updateProduct: (id: string, updates: repo.ProductUpdate) => Promise<void>; deleteProduct: (id: string) => Promise<void>;
  addOrder: (orderData: repo.NewOrder) => Promise<string>; updateOrder: (id: string, updates: Partial<Order>) => Promise<void>;
  archiveOrders: (ids: string[]) => Promise<number>; resetOrdersForNewPeriod: () => Promise<number>; deleteOrder: (id: string) => Promise<void>;
  addUser: (user: repo.NewManagedUser) => Promise<void>; updateUser: (user: repo.ManagedUserUpdate) => Promise<void>; updateProtectedCredentials: (user: repo.ProtectedCredentialsUpdate) => Promise<void>; deleteUser: (id: string) => Promise<void>;
  getProductsByCategory: (categoryId?: string) => Product[]; getOrdersByUser: (userId: string) => Order[]; subscribeRealtime: () => () => void; clearHistory: () => void;
}

async function remoteAudit(entry: Parameters<typeof writeAuditLog>[0]) {
  writeAuditLog(entry);
  try { await repo.writeAudit({ action: entry.action, actor_id: entry.actorId, actor_email: entry.actorEmail, entity: entry.entity, entity_id: entry.entityId, metadata: entry.metadata }); }
  catch (error) { writeAuditLog({ action: 'app.error', metadata: { source: 'remote_audit', message: String(error) } }); }
}

export const useDataStore = create<DataState>((set, get) => ({
  categories: [], products: [], orders: [], users: [], history: [], loading: false,
  ...
  deleteUser: async (id) => { await repo.deleteManagedUser(id); await remoteAudit({ action: 'settings.update', entity: 'user', entityId: id, metadata: { deleted: true } }); set({ users: get().users.filter((user) => user.id !== id) }); },

  getProductsByCategory: (categoryId) => { const visible = get().products.filter((product) => product.available && product.stock > 0); return categoryId ? visible.filter((product) => product.category_id === categoryId) : visible; },
  getOrdersByUser: (userId) => get().orders.filter((order) => order.user_id === userId),

  subscribeRealtime: () => {
    const supabase = requireSupabaseClient(); if (!appConfig.supabaseRealtimeEnabled) return () => undefined;
    let refreshTimer: ReturnType<typeof setTimeout> | undefined; let refreshInterval: ReturnType<typeof setInterval> | undefined;
    const scheduleRefresh = () => { if (refreshTimer) clearTimeout(refreshTimer); refreshTimer = setTimeout(() => { get().loadData({ silent: true }).catch((error) => writeAuditLog({ action: 'app.error', metadata: { source: 'realtime_refresh', message: String(error) } })); }, 150); };
    const refreshOnFocus = () => { if (document.visibilityState === 'visible') scheduleRefresh(); };
    let channel = supabase.channel('quickbite-db-changes');
    REALTIME_TABLES.forEach((table) => { channel = channel.on('postgres_changes', { event: '*', schema: 'public', table }, scheduleRefresh); });
    channel.subscribe((status, error) => { if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') writeAuditLog({ action: 'app.error', metadata: { source: 'realtime_subscription', status, message: error?.message } }); if (status === 'SUBSCRIBED') scheduleRefresh(); });
    document.addEventListener('visibilitychange', refreshOnFocus); window.addEventListener('focus', scheduleRefresh);
    if (appConfig.dataRefreshIntervalMs > 0) refreshInterval = setInterval(scheduleRefresh, appConfig.dataRefreshIntervalMs);
    return () => { if (refreshTimer) clearTimeout(refreshTimer); if (refreshInterval) clearInterval(refreshInterval); document.removeEventListener('visibilitychange', refreshOnFocus); window.removeEventListener('focus', scheduleRefresh); supabase.removeChannel(channel); };
  },
