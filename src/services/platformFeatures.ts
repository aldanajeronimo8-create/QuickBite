...
export async function getReorderItems(orderId: string) { const { data, error } = await supabase().from('order_items').select('product_id, quantity, price, product:products(id,name,price,stock,available,image_url,category_id)').eq('order_id', orderId); if (error) throw error; return (data ?? []).filter((item) => item.product && item.product.available && item.product.stock > 0); }
...
export async function getDailySummary(date = new Date().toISOString().slice(0, 10)) { const { data, error } = await supabase().from('daily_summaries').select('*').eq('business_date', date).maybeSingle(); if (error) throw error; return data; }
export async function assignStaffRole(userId: string, role: StaffRole) { const { data, error } = await supabase().from('staff_roles').upsert({ user_id: userId, role, updated_at: new Date().toISOString() }).select().single(); if (error) throw error; return data; }
export async function getStaffRole(userId: string) { const { data, error } = await supabase().from('staff_roles').select('role').eq('user_id', userId).maybeSingle(); if (error) throw error; return data?.role as StaffRole | undefined; }
export async function recordDemand(productId: string, quantity: number, source: 'order' | 'manual' | 'snapshot' = 'order') { const { error } = await supabase().from('demand_observations').insert({ product_id: productId, quantity, source }); if (error) throw error; }
export async function getDemandTrend(productId: string, days = 30) { const since = new Date(Date.now() - days * 86400000).toISOString(); const { data, error } = await supabase().from('demand_observations').select('quantity,observed_at').eq('product_id', productId).gte('observed_at', since).order('observed_at'); if (error) throw error; const values = (data ?? []).map((x) => Number(x.quantity)); if (!values.length) return { average: 0, peak: 0, trend: 0 }; const average = values.reduce((a, b) => a + b, 0) / values.length; const half = Math.max(1, Math.floor(values.length / 2)); const first = values.slice(0, half).reduce((a, b) => a + b, 0) / half; const last = values.slice(-half).reduce((a, b) => a + b, 0) / half; return { average, peak: Math.max(...values), trend: last - first }; }
export async function suggestPreparationQuantity(productId: string, safetyFactor = 1.2) { const trend = await getDemandTrend(productId, 30); return Math.max(0, Math.ceil((trend.average + Math.max(0, trend.trend)) * safetyFactor)); }
export async function recordAutomationJob(jobKey: string, status: 'running' | 'success' | 'failed', attempts = 1, errorMessage?: string) { const { data, error } = await supabase().from('automation_jobs').insert({ job_key: jobKey, status, attempts, finished_at: status === 'running' ? null : new Date().toISOString(), error_message: errorMessage ?? null }).select().single(); if (error) throw error; return data; }
