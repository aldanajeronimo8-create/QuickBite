...
/** 1. Real-time order status subscription. */
...
/** 2. Admin real-time order queue subscription. */
...
/** 4. Schedule an order for a configured pickup slot. */
...
/** 9. Rebuild a cart from a previous order. */
...
/** 10. Add/remove a product from favorites. */
...
/** 11. Read favorites with product details. */
...
/** 13. Product categories with available products. */
...
/** 14. Configure minimum/reorder stock thresholds. */
...
/** 15. Find products below their configured threshold. */
export async function listLowStockProducts() {
  const { data, error } = await supabase().from('products').select('id,name,stock,available,product_stock_settings(*)').order('stock');
  if (error) throw error;
  return (data ?? []).filter((p: any) => p.product_stock_settings?.[0] && p.stock <= p.product_stock_settings[0].minimum_stock);
}

/** 16. Read unresolved system alerts. */
...
/** 18. Dashboard daily sales metrics. */
...
/** 19. Generate/update today's operational summary. */
...
/** 20. Role assignment for staff, restricted by database RLS. */
...
/** 21. Read the current staff role. */
...
/** 22. Record observed demand for future forecasting. */
...
/** 23. Calculate simple demand trend from historical observations. */
...
/** 24. Suggest preparation quantity using recent average + positive trend. */
...
/** 25. Record an automation job result for backup/health/retry workflows. */
