...
export function buildPeriodReportWorkbook(orders: Order[], period: ReportPeriod) {
  ...
  const salesRowsData = salesRows(orders);
  writeTable(salesSheet, 0, ['N.º pedido', 'Fecha de compra', 'Hora de compra', 'Cliente', 'Correo', 'Documento', 'Estado pedido', 'Estado pago', 'Método de pago', 'Código recogida', 'Referencia de pago', 'Total pedido', 'Productos', 'Unidades', 'Tiempo estimado (min)'], salesRowsData, [18, 13, 10, 24, 30, 18, 20, 18, 18, 18, 24, 16, 11, 11, 18], [11], [12, 13, 14]);
  XLSX.utils.book_append_sheet(workbook, salesSheet, 'Ventas');

  if (period.mode === 'weekly') {
    const weekSheet = XLSX.utils.aoa_to_sheet([['Estructura de la semana'], ['Días totales', 7], ['Días con actividad', days.filter((day) => day.active).length], ['Días sin actividad', days.filter((day) => !day.active).length], ['Ventas confirmadas', confirmedSalesTotal]]);
    weekSheet['A1'] = { v: `Estructura · Semana ${period.weekNumber} de ${period.start.getUTCFullYear()}`, t: 's', s: titleStyle };
    writeTable(weekSheet, 5, ['Día', 'Fecha', 'Actividad', 'Ventas', 'Pedidos', 'Unidades'], days.map((d) => [d.label, d.date, d.active ? 'Sí' : 'No', d.sales, d.orders, d.units]), [18, 14, 14, 18, 14, 14], [3], [4, 5]);
