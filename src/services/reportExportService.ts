...
function totalSales(orders: Order[]) { return confirmed(orders).reduce((sum, order) => sum + Number(order.total), 0); }
...
function salesRows(orders: Order[]) { return orders.map((order) => [order.order_number, purchaseDate(order.created_at), purchaseTime(order.created_at), order.user?.full_name ?? 'Sin cliente', order.user?.email ?? '', order.user?.ti ?? '', statusLabel(order.status), statusLabel(order.payment_status), paymentLabel(order.payment_method), order.pickup_code ?? '', order.payment_reference ?? '', Number(order.total), Number(order.order_items?.length ?? 0), Number(order.order_items?.reduce((sum, item) => sum + item.quantity, 0) ?? 0), Number(order.estimated_minutes ?? 0)]); }
function writeTable(sheet: XLSX.WorkSheet, startRow: number, headers: string[], rows: unknown[][], widths: number[], currencyColumns: number[] = [], integerColumns: number[] = []) { headers.forEach((header, column) => { sheet[sheetCell(startRow, column)] = { v: header, t: 's', s: headerStyle }; }); rows.forEach((row, rowIndex) => row.forEach((value, column) => { const cell = sheetCell(startRow + 1 + rowIndex, column); sheet[cell] = { v: value as never, t: typeof value === 'number' ? 'n' : 's', s: rowIndex % 2 ? { fill: { fgColor: { rgb: 'F5F9FF' } } } : undefined }; if (currencyColumns.includes(column)) sheet[cell].z = currencyFormat; else if (integerColumns.includes(column)) sheet[cell].z = '0'; })); sheet['!cols'] = widths.map((wch) => ({ wch })); sheet['!autofilter'] = { ref: `A${startRow + 1}:${sheetCell(startRow + rows.length, headers.length - 1)}` }; sheet['!freeze'] = { xSplit: 0, ySplit: startRow + 1, state: 'frozen' }; }
function metricRows(period: ReportPeriod, orders: Order[]) { const confirmedOrders = confirmed(orders); const sales = totalSales(orders); return [['Período', period.label], ['Fecha inicial', dateFormatter.format(period.start)], ['Fecha final', dateFormatter.format(period.end)], ['Días totales', period.totalDays], ['Días con actividad', new Set(orders.map((order) => dateKeyInBogota(order.created_at))).size], ['Días sin actividad', Math.max(period.totalDays - new Set(orders.map((order) => dateKeyInBogota(order.created_at))).size, 0)], ['Ventas confirmadas', sales], ['Pedidos totales', orders.length], ['Pedidos confirmados', confirmedOrders.length], ['Pedidos rechazados', orders.filter((order) => order.payment_status === 'rejected').length], ['Unidades vendidas', totalUnits(orders)], ['Ticket promedio', confirmedOrders.length ? sales / confirmedOrders.length : 0]]; }

export function buildPeriodReportWorkbook(orders: Order[], period: ReportPeriod) {
  const sales = totalSales(orders);
  const days = period.days.map((day) => { const key = day.toISOString().slice(0, 10); const dayOrders = orders.filter((order) => dateKeyInBogota(order.created_at) === key); const confirmedDay = confirmed(dayOrders); return { day, key, label: weekdayFormatter.format(day), date: dateFormatter.format(day), active: dayOrders.length > 0, orders: dayOrders.length, sales: confirmedDay.reduce((sum, order) => sum + Number(order.total), 0), units: confirmedDay.reduce((sum, order) => sum + Number(order.order_items?.reduce((u, item) => u + Number(item.quantity), 0) ?? 0), 0) }; });
  ...
  summary['!cols'] = [{ wch: 26 }, { wch: 45 }, { wch: 20 }, { wch: 20 }];
  summary['!rows'] = [{ hpt: 30 }];
  for (let i = 1; i < summaryRows.length + 1; i += 1) { summary[`A${i + 1}`].s = labelStyle; summary[`B${i + 1}`].s = valueStyle; }
  summary['B8'].z = summary['B13'].z = currencyFormat;
  summary['B9'].z = summary['B10'].z = summary['B11'].z = summary['B12'].z = '0';
  XLSX.utils.book_append_sheet(workbook, summary, 'Resumen');

  const salesSheet = XLSX.utils.aoa_to_sheet([]);
  const sales = salesRows(orders);
  writeTable(salesSheet, 0, ['N.º pedido', 'Fecha de compra', 'Hora de compra', 'Cliente', 'Correo', 'Documento', 'Estado pedido', 'Estado pago', 'Método de pago', 'Código recogida', 'Referencia de pago', 'Total pedido', 'Productos', 'Unidades', 'Tiempo estimado (min)'], sales, [18, 13, 10, 24, 30, 18, 20, 18, 18, 18, 24, 16, 11, 11, 18], [11], [12, 13, 14]);
  XLSX.utils.book_append_sheet(workbook, salesSheet, 'Ventas');

  if (period.mode === 'weekly') {
    const weekSheet = XLSX.utils.aoa_to_sheet([['Estructura de la semana'], ['Días totales', 7], ['Días con actividad', days.filter((day) => day.active).length], ['Días sin actividad', days.filter((day) => !day.active).length]]);
    weekSheet['A1'] = { v: `Estructura · Semana ${period.weekNumber} de ${period.start.getUTCFullYear()}`, t: 's', s: titleStyle };
    writeTable(weekSheet, 4, ['Día', 'Fecha', 'Actividad', 'Ventas', 'Pedidos', 'Unidades'], days.map((d) => [d.label, d.date, d.active ? 'Sí' : 'No', d.sales, d.orders, d.units]), [18, 14, 14, 18, 14, 14], [3], [4, 5]);
    XLSX.utils.book_append_sheet(workbook, weekSheet, 'Estructura semanal');
  }

  if (period.mode === 'monthly') {
    const monthlySheet = XLSX.utils.aoa_to_sheet([['Estructura del mes'], ['Período', formatPeriodDateRange(period)], ['Días totales', period.totalDays], ['Días con actividad', days.filter((day) => day.active).length], ['Días sin actividad', days.filter((day) => !day.active).length]]);
    ...
    for (const group of groups) {
      ...
      for (const groupDay of days.filter((d) => group.days.some((g) => g.getTime() === d.day.getTime()))) {
        ...
        monthlySheet[sheetCell(cursor, 4)] = { v: groupDay.orders, t: 'n', z: '0' };
        monthlySheet[sheetCell(cursor, 5)] = { v: groupDay.units, t: 'n', z: '0' };
        cursor += 1;
      }
      cursor += 1;
    }
    ...
    const weeklyRows = groups.map((group) => { const groupDays = days.filter((d) => group.days.some((g) => g.getTime() === d.day.getTime())); return [group.weekNumber, dateFormatter.format(group.start), dateFormatter.format(group.end), groupDays.filter((d) => d.active).length, groupDays.reduce((sum, d) => sum + d.sales, 0), groupDays.reduce((sum, d) => sum + d.orders, 0), groupDays.reduce((sum, d) => sum + d.units, 0)]; });
    ...
    weeklyRows.forEach((_, r) => { weeksSheet[sheetCell(r + 1, 4)].z = currencyFormat; weeksSheet[sheetCell(r + 1, 0)].z = weeksSheet[sheetCell(r + 1, 3)].z = weeksSheet[sheetCell(r + 1, 5)].z = weeksSheet[sheetCell(r + 1, 6)].z = '0'; });
